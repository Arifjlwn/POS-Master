...
const fetchTenantHubData = async () => {
	...
	try {
		...
		Swal.fire({
			...
			text: err.response?.data?.error || 'Gagal memuat telemetri super admin .',
...
<template>
	<div class="p-4 sm:p-6 lg:p-8 bg-[#0B0F19] min-h-screen text-white font-sans overflow-x-hidden">
		...
		<TenantList :stores="filteredAndSortedStores" :is-loading="isLoading" @suspend="onSuspendStore" @activate="onActivateStore" />
	</div>
</template>
