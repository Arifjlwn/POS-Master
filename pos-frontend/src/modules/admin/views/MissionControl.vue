...
// Penampung data metrik utama platform
const metrics = ref({
    total_tenants: 0,
    active_tenants: 0,
    pending_tenants: 0,
...
// Penampung status kesehatan infrastruktur server
...
// Penampung daftar aktivitas log terbaru
const recentActivities = ref([]);

const isLoading = ref(false);
let telemetryInterval = null; // 🚀 Menyimpan ID interval untuk pembersihan lifecycle

// Fungsi terpusat untuk menarik seluruh data telemetri secara berkala
const loadTelemetryData = async (showSilently = false) => {
    // Jika showSilently true, proses loading berjalan di balik layar tanpa memicu animasi loading utama
    if (!showSilently) isLoading.value = true;
    
    try {
        const res = await api.get('/admin/dashboard-stats');
        
        // 1. Sinkronisasi Data Metrik Tenant
        const dataStats = res.data?.stats || res.data?.data;
        if (dataStats) {
            metrics.value = dataStats;
        }

        // 2. Sinkronisasi Data Kesehatan Server (Mengambil payload dari response backend Go)
        if (res.data?.server_health) {
            serverHealth.value = res.data.server_health;
        }

        // 3. Sinkronisasi Aliran Log Audit Terbaru
        if (res.data?.recent_activities) {
            recentActivities.value = res.data.recent_activities;
        }

    } catch (err) {
        // Hanya tampilkan pesan galat SweetAlert jika bukan panggilan otomatis (silent polling)
        if (!showSilently) {
            Swal.fire({
                icon: 'error',
        ...
        if (!showSilently) isLoading.value = false;
    }
};
...
onMounted(() => {
    ...
    // 🚀 AKTIVASI REALTIME AUTO-POLLING: Menarik data secara otomatis setiap 10 detik (10000ms)
    telemetryInterval = setInterval(() => {
        loadTelemetryData(true); // Panggil secara senyap (silent) agar tidak mengganggu pandangan pengguna
    }, 10000);
});

onUnmounted(() => {
    // 🚀 LIFECYCLE CLEANUP: Wajib menghapus interval saat user pindah page agar memori tidak bocor
    ...
    <div class="w-full min-h-screen bg-[#0B0F19] p-3 sm:p-5 lg:p-6 font-sans select-none text-white relative overflow-x-hidden">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 sm:mb-8 lg:mb-10 gap-4 relative z-10">
            <div class="w-full lg:w-auto">
                ...
                <h1 class="text-xl sm:text-2xl md:text-3xl font-black text-white tracking-tight break-words">
                    ARZURA MISSION CONTROL
                </h1>
                <p class="text-slate-500 font-bold text-[9px] sm:text-[10px] uppercase tracking-widest mt-1 leading-relaxed">
                    Realtime Multi-Tenant Telemetry Monitoring System
                </p>
            </div>

            <button 
                @click="loadTelemetryData(false)" 
                :disabled="isLoading" 
                class="w-full lg:w-auto justify-center px-5 py-3 bg-[#131B2E] border border-slate-800 hover:border-slate-700 active:scale-[0.98] disabled:opacity-50 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl flex items-center gap-2 shrink-0"
            >
        ...
        <div class="mt-6 bg-[#131B2E] border border-slate-800 rounded-2xl p-4 text-center relative z-10 shadow-md">
            <p class="text-slate-500 font-black text-[9px] sm:text-[10px] uppercase tracking-widest leading-relaxed">
