...
// State khusus untuk mengontrol buka-tutup Submenu Command Center bray
...
const commandCenterSubMenus = [
    { name: 'Realtime Telemetry', path: '/admin/dashboard' }, // Overview utama kita lebur di sini bray
...
<template>
    ...
    >
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 scrollbar-hide">
            ...
            <div>
                <button 
                    @click="toggleCommandCenter"
                    class="w-full flex items-center justify-between px-4 py-3.5 rounded-xl font-bold text-sm transition-all duration-200 group relative whitespace-nowrap"
                    :class="isChildActive ? 'bg-indigo-600/5 text-indigo-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'"
                >
                    ...
                    <svg 
                        :class="isCommandCenterOpen ? 'rotate-180 text-indigo-400' : 'text-slate-500'"
                ...
                <div v-show="isCommandCenterOpen" class="mt-1 pl-4 space-y-1 transition-all duration-300">
                    <router-link 
                        v-for="sub in commandCenterSubMenus" 
                        :key="sub.name" 
                        :to="sub.path"
                        @click="emit('close')"
                        class="flex items-center gap-3 px-6 py-2.5 rounded-xl font-bold text-xs transition-all duration-150 relative whitespace-nowrap"
                        :class="checkActive(sub.path) ? 'text-indigo-400 font-black' : 'text-slate-500 hover:text-white'"
                    >
