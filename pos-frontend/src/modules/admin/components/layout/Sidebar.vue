...
// State khusus untuk mengontrol buka-tutup Submenu Command Center
const isCommandCenterOpen = ref(true);

const toggleCommandCenter = () => {
	isCommandCenterOpen.value = !isCommandCenterOpen.value;
};

// Data menu utama yang tidak memiliki anak sub-menu
const mainMenuItems = [
	{
		name: 'Analytics',
...
// 🚀 EMBED DATA SUBMENU COMMAND CENTER KASTA TERTINGGI (5 PILAR PADAT)
const commandCenterSubMenus = [
	{ name: 'Realtime Telemetry', path: '/admin/dashboard' }, // Overview utama kita lebur di sini
	{ name: 'Live Radar Map', path: '/admin/command/map' },
	{ name: 'Live Transaction Feed', path: '/admin/command/transactions' },
	{ name: 'System Diagnostics', path: '/admin/command/diagnostics' }, // Gabungan server, db, & alert center
	{ name: 'Quick Access Hub', path: '/admin/command/quick-access' },
];

const checkActive = (path) => route.path === path;
const isChildActive = computed(() => commandCenterSubMenus.some((sub) => route.path === sub.path));
...
<template>
	...
	<aside :class="isOpen ? 'translate-x-0 ml-0' : '-translate-x-full lg:translate-x-0 lg:-ml-72'" class="fixed lg:static inset-y-0 left-0 z-50 w-72 bg-[#0B0F19] border-r border-slate-800 transform transition-all duration-300 ease-in-out flex flex-col shrink-0">
		...
		<div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 scrollbar-hide">
			<div>
				<button @click="toggleCommandCenter" class="w-full flex items-center justify-between px-4 py-3.5 rounded-xl font-bold text-sm transition-all duration-200 group relative whitespace-nowrap" :class="isChildActive ? 'bg-indigo-600/5 text-indigo-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'">
					<div v-if="isChildActive" class="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-indigo-500 rounded-r-full"></div>

					<div class="flex items-center gap-3">
						<svg class="w-5 h-5 shrink-0" :class="isChildActive ? 'text-indigo-400' : 'text-slate-500 group-hover:text-slate-300'" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
							<path
								stroke-linecap="round"
								stroke-linejoin="round"
								d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 12.03 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-.738.099-1.454.284-2.14" />
						</svg>
						<span>Command Center</span>
					</div>

					<svg :class="isCommandCenterOpen ? 'rotate-180 text-indigo-400' : 'text-slate-500'" class="w-4 h-4 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
					</svg>
				</button>

				<div v-show="isCommandCenterOpen" class="mt-1 pl-4 space-y-1 transition-all duration-300">
					<router-link v-for="sub in commandCenterSubMenus" :key="sub.name" :to="sub.path" @click="emit('close')" class="flex items-center gap-3 px-6 py-2.5 rounded-xl font-bold text-xs transition-all duration-150 relative whitespace-nowrap" :class="checkActive(sub.path) ? 'text-indigo-400 font-black' : 'text-slate-500 hover:text-white'">
						<div class="w-1.5 h-1.5 rounded-full" :class="checkActive(sub.path) ? 'bg-indigo-500 shadow-md shadow-indigo-500/50' : 'bg-slate-700'"></div>
						<span>{{ sub.name }}</span>
					</router-link>
				</div>
			</div>

			<router-link v-for="item in mainMenuItems" :key="item.name" :to="item.path" @click="emit('close')" class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-sm transition-all duration-200 group relative whitespace-nowrap" :class="checkActive(item.path) ? 'bg-indigo-600/10 text-indigo-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'">
