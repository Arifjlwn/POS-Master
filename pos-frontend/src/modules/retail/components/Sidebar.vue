...
// Inject semua data dan fungsi dari layer composable asli lu
const { route, sidebarOpen, openGroups, user, toggleGroup, logout } = useSidebar();

// 🚀 BACA PAKET SAAS DARI LOCAL STORAGE (REAKTIF)
...
// API BASE URL JAGA-JAGA BUAT LOGO
...
const triggerUpgrade = (fiturName, minLevel) => {
  ...
  Swal.fire({
    ...
    confirmButtonText: "Upgrade Sekarang", showCancelButton: true, cancelButtonText: "Nanti Dulu",
    confirmButtonColor: "#4f46e5", cancelButtonColor: "#94a3b8", customClass: { popup: "rounded-[32px]" },
  }).then((result) => {
    if (result.isConfirmed) router.push("/retail/account");
  });
};

// 🚀 FUNGSI SINKRON DATA DARI DB / MEMORY LOKAL
const updatePlanRealtime = () => { subPlan.value = localStorage.getItem("subscriptionPlan") || "basic"; };

// 🚀 GLOBAL GUARD + SYNC DATA PASCA LOGIN
onMounted(async () => {
  // 🎯 KUPING SAKTI (EVENT LISTENER REALTIME)
  ...
  if (role === "owner") {
    try {
      ...
      // ==============================================================
      // 🚀 FIX PENGAMAN CLOUD DISINI BRAY: Biar Gak Tabrakan Link Supabase!
      // ==============================================================
      if (data.logo_url) {
        // Kalau link database dari backend udah ngebawa format HTTPS internet cloud asli, langsung return mentah tanpa API_BASE_URL lokal bray!
        user.storeLogo = (data.logo_url.startsWith("http://") || data.logo_url.startsWith("https://")) ? data.logo_url : API_BASE_URL + data.logo_url;
      ...
      // Tembakkan ke composable biar sinkron
      ...
      if (isDead && route.path !== "/retail/account") router.push("/retail/account");
    } catch (e) {
      console.error("Gagal verifikasi masa aktif global & sinkronisasi logo:", e);
    }
  }
});

// Bersihkan kuping listener pas komponen dihancurkan biar ga memory leak
onUnmounted(() => {
    ...
    <header class="bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 sticky top-0 z-40 shadow-sm shrink-0 no-print">
      <div class="flex items-center gap-4 sm:gap-5">
        ...
        <div class="flex items-center gap-3">
          ...
          <div v-else class="font-black text-lg sm:text-xl text-slate-900 tracking-tighter leading-none"><span class="text-indigo-600">{{ user.storeName || "POS UMKM" }}</span></div>
      ...
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-3 sm:pl-4 sm:border-l border-slate-100">
          <div class="text-right hidden sm:block">
            <div class="text-xs font-black text-slate-800 uppercase leading-none">{{ user.name ? user.name.split(" ")[0] : "USER" }}</div>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'" class="fixed inset-y-0 left-0 w-[280px] sm:w-[320px] bg-white transform transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-50 flex flex-col border-r border-slate-100 h-full">
      <div class="p-6 sm:p-8 flex items-center justify-between bg-slate-50/50 border-b border-slate-100 shrink-0">
        <div class="flex flex-col">
          ...
          <div v-else class="font-black text-xl sm:text-2xl text-slate-900 tracking-tighter leading-none">{{ user.storeName || "NEXA POS" }}</div>
      ...
      <nav class="flex-1 px-4 sm:px-6 py-6 space-y-8 overflow-y-auto custom-scrollbar min-h-0">
        <div>
          <div class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] px-2 mb-4 flex items-center gap-2"><span class="w-1.5 h-1.5 bg-indigo-500 rounded-full shadow-[0_0_8px_rgba(99,102,241,0.6)]"></span>Operasional</div>
          <div class="space-y-1.5">
            ...
            <a v-if="planLevel < 2" href="#" @click.prevent="triggerUpgrade('Smart Attendance', 2)" class="nav-link-locked">
              ...
              <span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span>
            </a>
        ...
        <div>
          ...
          <div v-show="openGroups.stock" class="mt-2 space-y-1 ml-4 border-l-2 border-slate-100 pl-2">
            <router-link v-if="user.role === 'owner'" to="/retail/produk/master-produk" @click="sidebarOpen = false" class="sub-link hover:text-emerald-600 hover:bg-emerald-50 hover:border-emerald-500" :class="{ 'active-sub !text-emerald-700 !bg-emerald-50/80 !border-emerald-500': route.path.startsWith('/retail/produk/master-produk') }">Master Data Produk</router-link>
            <router-link to="/retail/produk/penerimaan-barang" @click="sidebarOpen = false" class="sub-link hover:text-emerald-600 hover:bg-emerald-50 hover:border-emerald-500" :class="{ 'active-sub !text-emerald-700 !bg-emerald-50/80 !border-emerald-500': route.path === '/retail/produk/penerimaan-barang' }">Terima Barang (PSB)</router-link>
            <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Stock Opname & Audit', 3)" class="sub-link-locked"><span class="flex-1">Stock Opname</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
            <router-link v-else to="/retail/stock-opname" @click="sidebarOpen = false" class="sub-link hover:text-emerald-600 hover:bg-emerald-50 hover:border-emerald-500" :class="{ 'active-sub !text-emerald-700 !bg-emerald-50/80 !border-emerald-500': route.path === '/retail/stock-opname' }">Stock Opname</router-link>
            <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Manajemen Retur & Waste', 3)" class="sub-link-locked"><span class="flex-1">Waste & Retur</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
            <router-link v-else to="/retail/produk/retur-barang" @click="sidebarOpen = false" class="sub-link hover:text-rose-600 hover:bg-rose-50 hover:border-rose-500" :class="{ 'active-sub !text-rose-700 !bg-rose-50/80 !border-rose-500': route.path.startsWith('/retail/produk/retur-barang') }">Waste & Retur</router-link>
          </div>
        </div>
        ...
        <div v-if="user.role !== 'staff'">
          ...
          <div v-show="openGroups.admin" class="mt-2 space-y-1 ml-4 border-l-2 border-slate-100 pl-2">
            <template v-if="user.role === 'owner'">
              <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Dashboard Analitik', 3)" class="sub-link-locked"><span class="flex-1">Dashboard Analitik</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
              ...
              <a v-if="planLevel < 2" href="#" @click.prevent="triggerUpgrade('Manajemen Karyawan', 2)" class="sub-link-locked"><span class="flex-1">Manajemen Karyawan</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
            ...
            <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Laporan Audit Lanjutan', 3)" class="sub-link-locked"><span class="flex-1">Laporan Hasil SO</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
            ...
            <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Laporan Audit Lanjutan', 3)" class="sub-link-locked"><span class="flex-1">Laporan Retur Barang</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
        ...
        <div>
          <div class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] px-2 mb-4 mt-6 flex items-center gap-2"><span class="w-1.5 h-1.5 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(59,130,246,0.6)]"></span>Sistem & Akun</div>
          <div class="space-y-1.5">
            <template v-if="user.role === 'owner'">
              <a v-if="planLevel < 3" href="#" @click.prevent="triggerUpgrade('Integrasi WhatsApp Notifier', 3)" class="nav-link-locked"><svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 21l1.65-3.8a9 9 0 1 1 3.4 2.9L3 21" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 10a.5.5 0 0 0 1 0V9a.5.5 0 0 0-1 0v1a5 5 0 0 0 5 5h1a.5.5 0 0 0 0-1h-1a.5.5 0 0 0 0 1" /></svg><span class="flex-1">Integrasi WA</span><span class="text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span></a>
              <router-link v-else to="/retail/settings/whatsapp" @click="sidebarOpen = false" class="nav-link group" :class="{ active: route.path === '/retail/settings/whatsapp' }"><svg class="icon group-hover:rotate-12 group-hover:text-emerald-500 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 21l1.65-3.8a9 9 0 1 1 3.4 2.9L3 21" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 10a.5.5 0 0 0 1 0V9a.5.5 0 0 0-1 0v1a5 5 0 0 0 5 5h1a.5.5 0 0 0 0-1h-1a.5.5 0 0 0 0 1" /></svg><span>Integrasi WA</span></router-link>
              <router-link to="/retail/settings" @click="sidebarOpen = false" class="nav-link group" :class="{ active: route.path === '/retail/settings' }"><svg class="icon group-hover:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg><span>Pengaturan Toko</span></router-link>
            </template>
            <router-link to="/retail/account" @click="sidebarOpen = false" class="nav-link group" :class="{ active: route.path === '/retail/account' }"><svg class="icon group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg><span>Akun Saya</span></router-link>
          </div>
        </div>
      </nav>
        ...
        <div class="flex items-center justify-between p-3 sm:p-4 bg-white rounded-2xl shadow-sm border border-slate-100 hover:border-slate-200 transition-colors">
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col max-w-[100px] sm:max-w-[120px]">
              <div class="text-xs font-black text-slate-800 uppercase leading-none truncate">{{ user.name || "USER" }}</div>
              <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 truncate">{{ user.role }}</span>
            </div>
          </div>
          <button @click="logout" class="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 hover:text-white hover:bg-rose-500 transition-all active:scale-95 group" title="Logout"><svg class="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg></button>
        </div>
      </div>
    </aside>
...
<style scoped>
.nav-link { @apply flex items-center gap-3.5 px-4 py-3.5 rounded-[16px] text-xs font-black tracking-tight transition-all duration-300 border-2 border-transparent text-slate-500 hover:bg-white hover:border-slate-100 hover:shadow-sm; }
.icon { @apply w-[18px] h-[18px] transition-transform duration-300; }
.nav-link.active { @apply bg-slate-900 text-white shadow-xl shadow-slate-200 border-slate-900 translate-x-1; }
.nav-link.active .icon { @apply text-indigo-400; }
.nav-link-locked { @apply flex items-center gap-3.5 px-4 py-3.5 rounded-[16px] text-xs font-black tracking-tight border-2 border-transparent text-slate-400 bg-slate-50/50 hover:bg-slate-100 cursor-not-allowed opacity-80; }
.sub-link-locked { @apply flex items-center gap-4 px-4 py-2.5 rounded-xl text-[11px] font-bold text-slate-400 border-l-[3px] border-transparent hover:bg-slate-50 cursor-not-allowed opacity-80; }
.group-btn { @apply w-full flex items-center justify-between px-2 py-2 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] transition-colors; }
.sub-link { @apply flex items-center gap-4 px-4 py-2.5 rounded-xl text-[11px] font-bold text-slate-500 transition-all duration-200 border-l-[3px] border-transparent; }
.active-sub { @apply shadow-sm; }
