			...
			<div class="flex items-center gap-4 sm:gap-5">
				...
				<div class="flex items-center gap-3">
					...
					<div v-else class="font-black text-lg sm:text-xl text-slate-900 tracking-tighter leading-none">
						<span class="text-indigo-600">{{ user.storeName || 'POS UMKM' }}</span>
					</div>
		...
		<aside :class="sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'" class="no-print fixed inset-y-0 left-0 w-[280px] sm:w-[320px] bg-white transform transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-50 flex flex-col border-r border-slate-100 h-full">
			<div class="p-6 sm:p-8 flex items-center justify-between bg-slate-50/50 border-b border-slate-100 shrink-0">
				<div class="flex flex-col">
					...
					<div v-else class="font-black text-xl sm:text-2xl text-slate-900 tracking-tighter leading-none">{{ user.storeName || 'ARZURA POS' }}</div>
					...
					<div class="space-y-1.5">
						<template v-if="user.role === 'owner'">
							...
							<router-link v-else to="/retail/settings/whatsapp" @click="sidebarOpen = false" class="nav-link group" :class="{ active: route.path === '/retail/settings/whatsapp' }">
								<svg class="icon group-hover:rotate-12 group-hover:text-emerald-500 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 21l1.65-3.8a9 9 0 1 1 3.4 2.9L3 21" />
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 10a.5.5 0 0 0 1 0V9a.5.5 0 0 0-1 0v1a5 5 0 0 0 5 5h1a.5.5 0 0 0 0-1h-1a.5.5 0 0 0 0 1" />
								</svg>
								<span>Integrasi WA</span>
							</router-link>
							<router-link to="/retail/settings" @click="sidebarOpen = false" class="nav-link group" :class="{ active: route.path === '/retail/settings' }">
...
<style scoped>
.nav-link {
	@apply flex items-center gap-3.5 px-4 py-3.5 rounded-[16px] text-xs font-black tracking-tight transition-all duration-300 border-2 border-transparent text-slate-500 hover:bg-white hover:border-slate-100 hover:shadow-sm;
}
.icon {
...
.nav-link-locked {
	@apply flex items-center gap-3.5 px-4 py-3.5 rounded-[16px] text-xs font-black tracking-tight border-2 border-transparent text-slate-400 bg-slate-50/50 hover:bg-slate-100 cursor-not-allowed opacity-80;
}
.sub-link-locked {
