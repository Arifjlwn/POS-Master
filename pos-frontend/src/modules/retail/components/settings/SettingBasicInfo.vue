...
// Inisialisasi Peta (Selalu Aktif & Bisa Digeser bray)
const initMap = () => {
...
const matchWilayah = (apiName, dbName) => {
    ...
    let api = cleanStr(apiName).replace(/KABUPATEN |KOTA |ADMINISTRASI /g, '').replace('DAERAH KHUSUS IBUKOTA JAKARTA', 'DKI JAKARTA').trim();
    let db = cleanStr(dbName).replace(/KABUPATEN |KOTA |ADMINISTRASI /g, '').replace('DAERAH KHUSUS IBUKOTA JAKARTA', 'DKI JAKARTA').trim();
    return api === db;
};
...
const loadKota = async (idProvinsi, targetName = null) => {
    ...
    try {
        ...
        if (targetName) {
            ...
            if (found) { regIds.value.kota = found.id; props.form.kota = found.name; return found.id; }
        }
        return null;
    } catch (err) { console.error(err); return null; } finally { isLoading.value.reg = false; }
};
...
const loadKecamatan = async (idKota, targetName = null) => {
    ...
    try {
        ...
        if (targetName) {
            ...
            if (found) { regIds.value.kecamatan = found.id; props.form.kecamatan = found.name; return found.id; }
        }
        return null;
    } catch (err) { console.error(err); return null; } finally { isLoading.value.dist = false; }
};
...
const loadKelurahan = async (idKecamatan, targetName = null) => {
    ...
    try {
        ...
        if (targetName) {
            ...
            if (found) { regIds.value.kelurahan = found.id; props.form.kelurahan = found.name; return found.id; }
        }
        return null;
    } catch (err) { console.error(err); return null; } finally { isLoading.value.vil = false; }
};
...
const initializeWilayah = async () => {
    ...
    // Centering awal dari database bray
...
const handleProvinsiChange = async () => {
    regIds.value.kota = ''; regIds.value.kecamatan = ''; regIds.value.kelurahan = '';
    listKota.value = []; listKecamatan.value = []; listKelurahan.value = [];
    props.form.kota = ''; props.form.kecamatan = ''; props.form.kelurahan = '';
...
const handleKotaChange = async () => {
    regIds.value.kecamatan = ''; regIds.value.kelurahan = '';
    listKecamatan.value = []; listKelurahan.value = [];
    props.form.kecamatan = ''; props.form.kelurahan = '';
...
const handleKecamatanChange = async () => {
    regIds.value.kelurahan = ''; listKelurahan.value = []; props.form.kelurahan = '';
    ...
    if (regIds.value.kecamatan) {
        ...
        setTimeout(() => { searchLocationByAddress(); }, 500);
    }
};
...
const handleKelurahanChange = () => {
    ...
    if (kel) {
        ...
        setTimeout(() => { searchLocationByAddress(); }, 300);
    }
};
