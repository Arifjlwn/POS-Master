...
<template>
    <div class="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden flex flex-col flex-1 h-[600px] xl:h-auto xl:sticky xl:top-24">
        ...
        <div v-if="cart.length > 0" class="p-6 bg-white border-t border-slate-100 shrink-0">
            <button @click="$emit('submit')" :disabled="isSubmitting" class="w-full bg-rose-600 hover:bg-slate-900 text-white py-5 rounded-[24px] font-black text-sm uppercase tracking-[0.2em] shadow-xl shadow-rose-200 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
