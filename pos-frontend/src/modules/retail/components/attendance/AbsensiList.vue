        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
            <div v-for="user in karyawan" :key="user.id || user.user_id"
                ...
                <div class="flex gap-2 md:gap-3 relative z-10 mt-auto">
                    ...
                    <template v-else>
                        <button v-if="!user.sudah_masuk" @click="$emit('mulai-absen', user.id || user.user_id, user.name, 'Masuk')"
                        ...
                        <button v-else-if="!user.sudah_pulang" @click="$emit('mulai-absen', user.id || user.user_id, user.name, 'Pulang')"
                            :disabled="(user.id || user.user_id) != currentUser.user_id"
