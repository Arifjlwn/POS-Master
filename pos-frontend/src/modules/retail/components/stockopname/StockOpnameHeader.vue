...
defineProps({ 
    activeTab: String,
    isOwner: Boolean, 
    isKlaimEligible: Boolean, 
    daysLeftKlaim: Number     
});
...
<template>
    <div>
        <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 rounded-[40px] p-8 md:p-10 mb-6 text-white shadow-2xl flex flex-col md:flex-row items-center justify-between relative overflow-hidden border border-white/10 select-none">
            ...
            <div class="z-10 text-center md:text-left">
                ...
                <p class="text-slate-300 font-bold text-[10px] uppercase tracking-[0.2em] flex items-center justify-center md:justify-start gap-2">
                    ...
                    Penyesuaian Stok Fisik vs Sistem Gudang
                </p>
            </div>

            <div class="z-10 mt-6 md:mt-0 flex items-center gap-3 bg-amber-500/20 px-5 py-3 rounded-2xl border border-amber-500/30 backdrop-blur-sm">
                ...
                <div class="flex flex-col text-left">
                    <span class="text-[9px] font-black text-amber-200 uppercase tracking-widest leading-none">Metode Audit</span>
                    <span class="text-sm font-black text-amber-400 uppercase tracking-widest mt-0.5">
        ...
        <div class="flex justify-end mb-8 no-print">
            <div class="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex gap-2">
                ...
                <button @click="$emit('update:activeTab', 'SO')" 
                        ...
                        class="px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95">
                    Audit Reguler
                </button>
                ...
                <button v-if="isKlaimEligible"
                        ...
                        class="px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2.5 active:scale-95">
                    ...
                    <span :class="activeTab === 'KLAIM' ? 'bg-amber-600 text-white shadow-inner' : 'bg-amber-100 text-amber-600'" 
                          class="px-2.5 py-1 rounded-md text-[9px] font-black tracking-widest ml-1 transition-colors">
                ...
                <button v-else
                        disabled
                        class="px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2 bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200 select-none"
                        title="Masa tenggat klaim 7 hari pasca opname telah kedaluwarsa atau berkas penyesuaian sudah diajukan.">
