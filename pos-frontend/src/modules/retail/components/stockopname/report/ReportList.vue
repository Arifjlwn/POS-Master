...
// 🚀 FIX LOGIC: Deteksi arah kerugian/surplus asli berdasarkan kuantitas selisih barang
		...
		<div class="overflow-x-auto custom-scrollbar max-h-[600px] overflow-y-auto">
			<table class="w-full text-left">
				<tbody class="divide-y divide-slate-50">
					...
					<tr v-for="r in reports" :key="r.so.id" @click="$emit('select', r)" class="cursor-pointer transition-all group relative" :class="selectedDetail?.so?.id === r.so.id ? 'bg-indigo-50 border-l-4 border-l-indigo-600' : 'hover:bg-slate-50 border-l-4 border-l-transparent'">
						<td class="p-5">
							...
							<div class="mt-3 flex items-center gap-2">
								<span v-if="getFinansialStatus(r.so.details) === 'MINUS'" class="bg-red-100 text-red-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">RUGI / MINUS</span>
								<span v-else-if="getFinansialStatus(r.so.details) === 'PLUS'" class="bg-emerald-100 text-emerald-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">SURPLUS / PLUS</span>
								<span v-else class="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">BALANCE</span>
