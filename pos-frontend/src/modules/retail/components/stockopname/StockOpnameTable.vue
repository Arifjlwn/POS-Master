            ...
            <table class="w-full text-left whitespace-nowrap border-collapse min-w-[600px]">
                ...
                <tbody class="divide-y divide-slate-50">
                    <tr v-if="cartSO.length === 0">
                        <td :colspan="(isOwner || soStep === 'REVIEW') ? 5 : 3" class="p-20 text-center">
                    ...
                    <template v-for="(item, index) in cartSO" :key="index">
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td v-if="isOwner || soStep === 'REVIEW'" class="p-6 text-center">
                                <div v-if="(item.actual_qty - item.system_qty) > 0" class="inline-flex flex-col items-center justify-center w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 font-black border border-emerald-200">
                                    +{{ item.actual_qty - item.system_qty }}
                                </div>
                                <div v-else-if="(item.actual_qty - item.system_qty) < 0" class="inline-flex flex-col items-center justify-center w-12 h-12 rounded-full bg-rose-50 text-rose-600 font-black border border-rose-200">
                                    {{ item.actual_qty - item.system_qty }}
                                </div>
                        ...
                        <tr v-if="(isOwner || soStep === 'REVIEW') && (item.actual_qty - item.system_qty) !== 0" class="bg-amber-50/30">
                            <td :colspan="isOwner || soStep === 'REVIEW' ? 5 : 3" class="px-6 py-4 border-t border-dashed border-amber-200">
                                <div class="flex items-start gap-4">
                                    <div class="mt-2 shrink-0"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></div>
                                    <div class="flex-1">
                                        <label class="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1.5 block">Keterangan / Bukti Selisih (Wajib)</label>
                                        <input v-model="item.alasan" type="text" :placeholder="(item.actual_qty - item.system_qty) > 0 ? 'Contoh: Nyempil di bawah rak...' : 'Contoh: Rusak dimakan tikus...'" class="w-full p-3 bg-white border border-amber-200 focus:border-amber-500 rounded-xl outline-none font-bold text-xs text-amber-900 shadow-sm placeholder:text-amber-300">
                                    </div>
                                </div>
                            </td>
                        </tr>
