    ...
    <table class="w-full text-left whitespace-nowrap border-collapse">
        <thead>
            <tr class="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
                ...
                <th class="p-6 text-center bg-blue-50/30 text-blue-600">Total Masuk<br>(Satuan Dasar)</th>
                <th class="p-6 text-right">Harga Total Nota<br>(Sesuai Kemasan Terbesar)</th>
                <th v-if="isOwner" class="p-6 text-right">HPP Baru<br>(Per Satuan Dasar)</th>
            ...
            <tr v-for="(item, index) in cartLPB" :key="item.product_id" 
                ...
                <td class="p-6">
                    ...
                    <div class="mt-1 flex flex-wrap gap-1">
                        ...
                        <span v-else class="px-2 py-0.5 bg-slate-100 border border-slate-200 text-slate-500 rounded text-[9px] font-black uppercase tracking-widest">
                            Eceran ({{ item.satuan_dasar }})
                        </span>
                    </div>
                ...
                <td class="p-6 min-w-[200px]"> 
                    ...
                    <div class="text-right mt-1.5 text-[8px] font-black text-slate-400 uppercase italic">
                        *Harga 1 {{ item.has_satuan_besar ? (item.satuan_besar || 'KEMASAN BESAR') : (item.satuan_dasar || 'PCS') }}
                    </div>
                </td>
                ...
                <td v-if="isOwner" class="p-6 text-right">
                    <div class="text-base font-black tracking-tight text-emerald-600" :class="{ 'text-red-600': hitungModalPerPcs(item) > item.harga_jual_saat_ini }">
                        Rp {{ formatNumber(hitungModalPerPcs(item)) }}
                    </div>
