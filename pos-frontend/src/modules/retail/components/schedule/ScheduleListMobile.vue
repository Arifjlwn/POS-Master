            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div v-for="d in mingguJadwal" :key="d.tanggal" class="bg-slate-50/80 p-3 rounded-xl border border-slate-100 flex flex-col items-center justify-between">
                    ...
                    <select 
                        ...
                        :class="{
                            'bg-emerald-50 text-emerald-700 border-emerald-200': formJadwal[emp.public_id || emp.id][d.tanggal].includes('Shift 1'),
                            'bg-blue-50 text-blue-700 border-blue-200': formJadwal[emp.public_id || emp.id][d.tanggal].includes('Shift 2'),
                            'bg-purple-50 text-purple-700 border-purple-200': formJadwal[emp.public_id || emp.id][d.tanggal].includes('Middle'),
                            'bg-slate-100 text-slate-400 border-slate-200': formJadwal[emp.public_id || emp.id][d.tanggal] === 'OFF',
                            'bg-slate-800 text-white border-slate-900': formJadwal[emp.public_id || emp.id][d.tanggal].includes('Shift 3')
                        }">
                        ...
                        <slot v-if="formJadwal[emp.public_id || emp.id][d.tanggal]">
                            <option hidden :value="formJadwal[emp.public_id || emp.id][d.tanggal]">
                                {{ formJadwal[emp.public_id || emp.id][d.tanggal].substring(0,7) }}
                            </option>
                        </slot>
                    </select>

                    <div v-if="formJadwal[emp.public_id || emp.id][d.tanggal] !== 'OFF'" class="w-full mt-1.5 flex justify-center">
                        <span v-if="formJadwal[emp.public_id || emp.id][d.tanggal].includes('(Pending)')" class="text-[8px] font-black uppercase text-amber-600 flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" class="w-2 h-2" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg> Pending</span>
                        <span v-else-if="formJadwal[emp.public_id || emp.id][d.tanggal].includes('(Approved)')" class="text-[8px] font-black uppercase text-emerald-600 flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" class="w-2 h-2" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg> Approved</span>
                        <span v-else-if="formJadwal[emp.public_id || emp.id][d.tanggal].includes('(Rejected)')" class="text-[8px] font-black uppercase text-red-600 flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" class="w-2 h-2" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg> Rejected</span>
                    </div>
                </div>
            </div>
        </div>
