                        ...
                        <td v-for="d in mingguJadwal" :key="d.tanggal" class="p-4 text-center border-l border-slate-50">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <div v-if="currentUser.role === 'owner' && formJadwal[emp.id || emp.user_id][d.tanggal].includes('(Pending)')" class="flex gap-2 mt-1 justify-center w-full">
                                    <button @click="emit('handle-approval', emp.id || emp.user_id, d.tanggal, 'approve')" class="flex-1 p-2 rounded-xl bg-emerald-100 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all shadow-sm border border-emerald-200 flex items-center justify-center" title="Setujui">
                                    ...
                                    <button @click="emit('handle-approval', emp.id || emp.user_id, d.tanggal, 'reject')" class="flex-1 p-2 rounded-xl bg-red-100 text-red-600 hover:bg-red-500 hover:text-white transition-all shadow-sm border border-red-200 flex items-center justify-center" title="Tolak">
