...
const emit = defineEmits(['update:isMobileCartOpen', 'show-held', 'hold-order', 'clear-cart', 'decrease-qty', 'increase-qty', 'validate-qty', 'set-nominal', 'set-payment', 'format-rupiah', 'checkout', 'toggle-uom']);

// 🚀 CLUSTERING ENGINE: Grouping utama Tunai vs Non-Tunai bray bray
const mainPaymentGroup = ref('Cash'); // Pilihan: 'Cash' atau 'Non-Cash'

// Sinkronisasi awal jika parameter paymentMethod dari bapaknya berubah bray
watch(
	() => props.paymentMethod,
	(newMethod) => {
		if (newMethod === 'Cash') {
			...
			mainPaymentGroup.value = 'Non-Cash';
		}
	},
	{ immediate: true }
);

const setMainGroup = (group) => {
	mainPaymentGroup.value = group;
	if (group === 'Cash') {
		emit('set-payment', 'Cash');
	} else {
		// Default otomatis sub non-tunai pertama kali diklik adalah QRIS
		emit('set-payment', 'QRIS');
	}
};

const filterQtyKeyboard = (event, item) => {
	// Izinkan angka dan karakter titik/koma desimal murni bray
	let cleanVal = event.target.value.replace(/[^0-9.]/g, '');

	// Cegah double titik desimal penipu bray
	const points = cleanVal.split('.');
	if (points.length > 2) {
		cleanVal = points[0] + '.' + points.slice(1).join('');
	}

	item.qty = cleanVal; // Biarkan menampung string desimal sementara pas diketik
};
</script>

<template>
	<div :class="[isMobileCartOpen ? 'fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex justify-end transition-opacity duration-300' : 'hidden lg:flex flex-col shrink-0 w-[300px] xl:w-[360px] 2xl:w-[400px]']">
		<div :class="[isMobileCartOpen ? 'w-[85%] sm:w-[340px] h-full max-h-screen translate-x-0 transition-transform duration-300 ease-out shadow-[-10px_0_30px_rgba(0,0,0,0.1)]' : 'w-full h-full']" class="bg-white lg:rounded-[24px] lg:border border-slate-200 flex flex-col h-full overflow-hidden shadow-lg">
			...
			<div class="p-2 xl:p-3 flex-1 overflow-y-auto bg-white custom-scrollbar min-h-[120px] relative">
				...
				<div v-for="item in cart" :key="item.id + '_' + item.selected_uom" class="flex flex-col mb-2 p-2 xl:p-3 bg-slate-50/50 rounded-xl border border-slate-100 shadow-sm relative group">
					<div class="flex justify-between items-start mb-1.5">
						<div class="flex-1 pr-1 xl:pr-2">
							...
							<div class="mt-1 flex flex-wrap items-center gap-1 xl:gap-1.5">
								...
								<button v-if="item.has_grosir" @click="emit('toggle-uom', item)" class="px-1.5 py-0.5 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded text-[7px] xl:text-[8px] font-black uppercase tracking-widest flex items-center justify-center transition-colors">Ubah</button>
							</div>
						</div>
						<div class="font-black text-[11px] xl:text-xs text-indigo-700 whitespace-nowrap text-right">Rp {{ (item.price * item.qty).toLocaleString('id-ID') }}</div>
					</div>
					...
					<div class="flex justify-between items-center">
						...
						<div class="flex items-center bg-white rounded-lg p-0.5 border border-slate-200 shadow-sm">
							...
							<input type="text" :value="item.qty" @input="filterQtyKeyboard($event, item)" @change="emit('validate-qty', item)" class="w-8 xl:w-10 text-center text-[11px] xl:text-xs font-black text-slate-800 bg-transparent border-none focus:ring-0 p-0" />
					...
					<div v-if="['KG', 'LITER'].includes(item.selected_uom?.toUpperCase())" class="flex gap-1 mt-2 pt-2 border-t border-slate-100 border-dotted animate-[fadeInUp_0.15s_ease-out]">
						<button
							type="button"
							@click.stop="
								item.qty = 0.25;
								validateQty ? validateQty(item) : emit('validate-qty', item);
							"
							:class="Number(item.qty) === 0.25 ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'"
							class="flex-1 py-1 rounded-md text-[8px] font-black uppercase border transition-all text-center active:scale-95 shadow-sm">
							1/4 {{ item.selected_uom }}
						</button>
						...
						<button
							...
							@click.stop="
								...
								validateQty ? validateQty(item) : emit('validate-qty', item);
							"
							:class="Number(item.qty) === 0.5 ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'"
							class="flex-1 py-1 rounded-md text-[8px] font-black uppercase border transition-all text-center active:scale-95 shadow-sm">
							1/2 {{ item.selected_uom }}
						</button>
						...
						<button
							...
							@click.stop="
								...
								validateQty ? validateQty(item) : emit('validate-qty', item);
							"
							:class="Number(item.qty) === 0.75 ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'"
							class="flex-1 py-1 rounded-md text-[8px] font-black uppercase border transition-all text-center active:scale-95 shadow-sm">
							3/4 {{ item.selected_uom }}
						</button>
						...
						<button
							...
							@click.stop="
								...
								validateQty ? validateQty(item) : emit('validate-qty', item);
							"
							:class="Number(item.qty) === 1 ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200'"
							class="flex-1 py-1 rounded-md text-[8px] font-black uppercase border transition-all text-center active:scale-95 shadow-sm">
							1 {{ item.selected_uom }}
						</button>
					</div>
			...
			<div class="p-2 md:p-3 bg-white border-t border-slate-100 shadow-[0_-5px_15px_-5px_rgba(0,0,0,0.05)] shrink-0 z-10 lg:rounded-b-[24px]">
				<div class="mb-2">
					<div class="grid grid-cols-2 gap-2">
						<button type="button" @click="setMainGroup('Cash')" :class="mainPaymentGroup === 'Cash' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100 border-indigo-600' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'" class="py-2 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all border flex justify-center items-center gap-1.5 active:scale-95">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
								<path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
							</svg>
							Tunai
						</button>
						<button type="button" @click="setMainGroup('Non-Cash')" :class="mainPaymentGroup === 'Non-Cash' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100 border-indigo-600' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'" class="py-2 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all border flex justify-center items-center gap-1.5 active:scale-95">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
				...
				<div v-show="mainPaymentGroup === 'Non-Cash'" class="mb-2 bg-slate-50 p-1 rounded-xl border border-slate-200/60 animate-[fadeInUp_0.15s_ease-out]">
					<div class="grid grid-cols-2 gap-1">
						<button type="button" @click="emit('set-payment', 'QRIS')" :class="paymentMethod === 'QRIS' ? 'bg-white text-indigo-600 shadow-sm border-slate-200 font-black' : 'text-slate-400 font-bold hover:bg-white/50'" class="py-1.5 rounded-lg text-[9px] uppercase tracking-widest border border-transparent transition-all flex items-center justify-center gap-1">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
								<path stroke-linecap="round" stroke-linejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
							</svg>
							QRIS
						</button>
						<button type="button" @click="emit('set-payment', 'Debit')" :class="paymentMethod === 'Debit' ? 'bg-white text-indigo-600 shadow-sm border-slate-200 font-black' : 'text-slate-400 font-bold hover:bg-white/50'" class="py-1.5 rounded-lg text-[9px] uppercase tracking-widest border border-transparent transition-all flex items-center justify-center gap-1">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
				...
				<div class="space-y-0.5 bg-slate-50 p-1.5 md:p-2 rounded-xl border border-slate-100 mb-2">
				...
				<div v-show="paymentMethod === 'QRIS'" class="bg-indigo-50 p-1.5 rounded-xl border border-indigo-100 flex items-center justify-between animate-[fadeInUp_0.2s_ease-out] mb-2">
					<div class="flex items-center gap-2">
						...
						<div class="text-[8px] font-black text-indigo-800 uppercase tracking-widest leading-none">
							Uang Pas
							<br />
							<span class="text-indigo-500 font-bold">Otomatis</span>
						</div>
					</div>
					<div class="text-right">
						<div class="text-[8px] font-black text-indigo-600 uppercase tracking-widest">Satelit QRIS Merchant</div>
					</div>
				</div>

				<div v-show="paymentMethod === 'Debit'" class="bg-blue-50 p-1.5 rounded-xl border border-blue-100 flex items-center justify-between animate-[fadeInUp_0.2s_ease-out] mb-2">
					<div class="flex items-center gap-2">
						...
						<div class="text-[8px] font-black text-blue-800 uppercase tracking-widest leading-none">
							Uang Pas
							<br />
							<span class="text-blue-500 font-bold">Otomatis</span>
						</div>
					</div>
					<div class="text-right"><div class="text-[8px] font-black text-blue-600 uppercase tracking-widest">Mutasi Rekening</div></div>
				</div>

				<div v-show="paymentMethod === 'Cash'" class="animate-[fadeInUp_0.2s_ease-out] mb-2">
					<div class="flex overflow-x-auto gap-1 custom-scrollbar pb-1.5">
						<button v-for="nom in [100000, 50000, 20000, 10000, 5000, 1000, 500, 100]" :key="nom" type="button" @click="emit('set-nominal', nom)" class="shrink-0 px-3 py-1.5 bg-white border border-slate-200 hover:border-indigo-400 hover:text-indigo-600 rounded-lg font-black text-[9px] text-slate-600 uppercase transition-all shadow-sm active:scale-95">
							{{ nom >= 1000 ? nom / 1000 + ' Rb' : nom }}
						</button>
					</div>
				</div>

				<div class="relative z-20 flex justify-between items-center bg-slate-50 p-1.5 rounded-xl border border-slate-200 focus-within:border-indigo-500 transition-all shadow-sm mb-2">
					<span class="font-black text-[10px] text-slate-600 uppercase tracking-widest pl-1">Input Bayar</span>
					<div class="flex-1 flex gap-1 ml-2">
						...
						<button v-show="paymentMethod === 'Cash'" type="button" @click="emit('set-nominal', 0)" class="w-8 h-[34px] bg-rose-50 border border-rose-200 hover:bg-rose-100 text-rose-600 rounded-lg flex items-center justify-center font-black transition-colors active:scale-95">C</button>
					</div>
				</div>

				<div class="flex justify-between items-center px-1 mb-2">
					<span class="font-black text-[9px] text-slate-400 uppercase tracking-widest">Kembalian</span>
					<span class="text-xs md:text-sm font-black" :class="kembalian >= 0 && paymentMethod === 'Cash' ? 'text-emerald-500' : 'text-red-500'">Rp {{ paymentMethod === 'Cash' ? (kembalian ? kembalian.toLocaleString('id-ID') : '0') : '0' }}</span>
				</div>

				<button @click="emit('checkout')" :disabled="cart.length === 0 || (paymentMethod === 'Cash' && payAmount < totalBelanja) || isProcessingCheckout" class="w-full text-white font-black py-2.5 md:py-3 rounded-xl transition-all flex justify-center items-center gap-2 disabled:opacity-50 shadow-md text-xs uppercase tracking-[0.1em]" :class="paymentMethod === 'Cash' ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-indigo-600 hover:bg-indigo-700'">
					<template v-if="isProcessingCheckout">
						<div class="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
						Memproses...
					</template>
					<template v-else>
						{{ paymentMethod === 'QRIS' ? 'Generate QRIS Nota' : paymentMethod === 'Debit' ? 'Konfirmasi Transfer' : 'Proses Nota Tunai' }}
					</template>
				</button>
			</div>
		</div>
