...
<template>
  <div
    :class="
      isMobileCartOpen
        ? 'fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex justify-end transition-all'
        : 'hidden lg:flex flex-col shrink-0 w-full max-w-[320px] xl:max-w-[380px] 2xl:max-w-[420px]'
    "
  >
    <div
      :class="
        isMobileCartOpen
          ? 'w-[85%] sm:w-[360px] h-full animate-slide-in-right'
          : 'w-full h-full'
      "
      class="bg-white lg:rounded-[32px] shadow-2xl border-l lg:border border-slate-100 flex flex-col overflow-hidden"
    >
      <div
        class="lg:hidden p-4 bg-indigo-900 text-white flex justify-between items-center shrink-0"
      >
        <h2
          class="font-black tracking-widest uppercase text-sm flex items-center gap-2"
        >
          <svg
        ...
        <button
          @click="emit('update:isMobileCartOpen', false)"
          class="bg-white/20 p-2 rounded-xl text-white hover:bg-rose-500 transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-5 h-5"
      ...
      <div
        class="p-4 md:p-5 border-b border-slate-100 bg-slate-50/80 hidden lg:flex justify-between items-center shrink-0"
      >
        <h2
          class="text-sm md:text-base font-black text-slate-800 flex items-center gap-2 uppercase tracking-widest"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="flex gap-1.5 md:gap-2">
          <button
            ...
            class="p-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-xl transition-colors relative"
            title="Lihat Pesanan Tertunda"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            <span
              class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[8px] font-black text-white shadow-md"
          ...
          <button
            ...
            class="p-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl transition-colors disabled:opacity-50"
            title="Hold Pesanan Ini"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-4 h-4 md:w-5 md:h-5"
          ...
          <button
            ...
            :disabled="cart.length === 0"
            class="p-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl transition-colors disabled:opacity-50"
      ...
      <div
        class="p-3 md:p-4 flex-1 overflow-y-auto bg-white custom-scrollbar min-h-0"
      >
        <div v-if="cart.length > 0" class="flex gap-2 lg:hidden mb-4">
          <button
            @click="emit('hold-order')"
            class="flex-1 py-2 bg-amber-50 text-amber-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-amber-100"
          >
            Hold Order
          </button>
          <button
            @click="emit('clear-cart')"
            class="flex-1 py-2 bg-rose-50 text-rose-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-rose-100"
          >
            Bersihkan
          </button>
        </div>
        ...
        <div
          v-if="cart.length === 0"
          class="h-full flex flex-col items-center justify-center opacity-30 py-12"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-12 h-12 md:w-16 md:h-16 mb-4 text-slate-500"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="1.5"
          ...
          <p
            class="text-slate-600 font-black text-xs md:text-sm uppercase tracking-widest"
          >
            Keranjang Kosong
          </p>
        </div>
        ...
        <div
          v-for="item in cart"
          :key="item.id + '_' + item.selected_uom"
          class="flex flex-col mb-3 p-3 bg-slate-50/50 rounded-xl md:rounded-2xl border border-slate-100 shadow-sm hover:border-indigo-200 transition-colors relative group"
        >
          <div class="flex justify-between items-start mb-2">
            <div class="flex-1 pr-2">
              <h3
                class="font-bold text-[10px] md:text-[11px] text-slate-800 leading-tight line-clamp-2 uppercase"
              >
                {{ item.name }}
              </h3>
              ...
              <div class="mt-1.5 flex flex-wrap items-center gap-1.5">
                ...
                <button
                  v-if="item.has_grosir"
                  @click="emit('toggle-uom', item)"
                  class="px-1.5 py-0.5 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded text-[8px] font-black uppercase tracking-widest flex items-center justify-center gap-1 transition-colors"
                  title="Ganti Satuan"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <div
              class="font-black text-[11px] md:text-xs text-indigo-700 whitespace-nowrap text-right"
            >
          ...
          <div class="flex justify-between items-center mt-1">
            <p class="text-[9px] md:text-[10px] font-bold text-slate-400">
              @ Rp {{ item.price.toLocaleString('id-ID') }}
            </p>
            <div
              class="flex items-center bg-white rounded-lg md:rounded-xl p-1 border border-slate-200 shadow-sm"
            >
              <button
                @click="emit('decrease-qty', item)"
                class="w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-md md:rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-600 font-black transition-colors"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-4 h-4"
              ...
              <input
                ...
                class="w-8 md:w-10 text-center text-xs font-black text-slate-800 bg-transparent border-none focus:ring-0 p-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              <button
                @click="emit('increase-qty', item)"
                class="w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-md md:rounded-lg text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 font-black transition-colors"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-4 h-4"
