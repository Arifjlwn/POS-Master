...
const modalAwal = computed(() => {
	return props.lastClosingData?.session?.modal_awal || props.lastClosingData?.session?.ModalAwal || props.lastClosingData?.data?.session?.modal_awal || props.currentSession?.modal_awal || props.currentSession?.ModalAwal || 0;
});
		...
		<div class="bg-white rounded-[24px] md:rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl border-[6px] md:border-[8px] border-slate-800 flex flex-col max-h-[95vh] md:max-h-[90vh]">
			...
			<div class="p-4 md:p-6 grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3 overflow-y-auto custom-scrollbar flex-1 bg-slate-50">
				<div
					...
					:key="denon.key"
					class="bg-white p-2 md:p-3 rounded-xl border-2 border-slate-200 focus-within:border-indigo-500 shadow-sm transition-all">
					...
					<input type="text" inputmode="numeric" :value="pecahan[denon.key] || ''" @input="sanitizePecahanInput($event, denon.key)" class="w-full bg-transparent text-lg font-black text-slate-800 outline-none" placeholder="0" />
				</div>
			</div>
		...
		<div class="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-2xl w-full max-w-sm flex flex-col max-h-[90vh] border-[6px] md:border-[8px] border-slate-800 print:border-none print:shadow-none print:max-h-none print:max-w-none print:rounded-none print:m-0 print:p-0">
			<div class="overflow-y-auto custom-scrollbar bg-white p-2 mx-auto print:overflow-visible print:mx-0 print:px-3 print:py-2 select-none" id="print-area">
				<div class="text-center mb-4 font-mono leading-none">
					...
					<div v-else class="font-black text-[12px] uppercase mb-1 italic tracking-tighter">{{ storeData?.nama_toko || storeData?.NamaToko || 'ARZURA POS STORE' }}</div>
					<p class="text-[9px] font-black uppercase tracking-tight opacity-100 leading-tight px-1">
						...
						<br />
						{{ storeData?.kelurahan || storeData?.Kelurahan || '' }} {{ storeData?.kecamatan || storeData?.Kecamatan || '' }}
						<br />
				...
				<div class="text-[9px] uppercase font-mono font-black mb-2 leading-tight space-y-0.5">
					...
					<div class="text-center font-black my-3 tracking-[0.15em] border-y border-black py-1 uppercase">
						SLIP PENJUALAN
						<br />
						TUTUP SHIFT KASIR
					</div>
					<div class="flex justify-between">
						<span>KASIR: {{ currentUser?.name || 'OPERATOR' }}</span>
						<span>STASIUN: POS-{{ currentSession?.station_number || currentSession?.StationNumber || '01' }}</span>
					</div>
				</div>
