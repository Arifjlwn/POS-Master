...
onMounted(() => {
    // Paksa kotak input langsung kedip pas halaman kasir pertama kali dimuat
    if (searchInputRef.value) {
        searchInputRef.value.focus();
    }
});

// Fungsi pembantu untuk memastikan kursor ga kabur pas kasir ga sengaja klik area kosong katalog
...
<template>
    <div @click="forceFocusScanner" class="flex-1 flex flex-col min-h-0 w-full lg:w-8/12 xl:w-9/12 transition-all duration-300 no-print">
        ...
        <div class="flex gap-2 md:gap-3 shrink-0 mb-3 md:mb-4 items-stretch h-12 md:h-14">
            <div class="relative flex-1 group h-full">
                ...
                <input
                    ...
                    placeholder="Cari Nama Produk / Scan Barcode SKU..."
        ...
        <div v-else class="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
                <div v-for="product in filteredProducts" :key="product.id" @click.stop="emit('add-to-cart', product)"
                    ...
                    <div class="relative pt-2 px-2 md:pt-3 md:px-3">
                        ...
                        <div v-else-if="product.stock <= 3" class="absolute top-3 right-3 md:top-5 md:right-5 text-[8px] md:text-[9px] font-black px-1.5 md:px-2 py-0.5 md:py-1 rounded-md shadow-sm bg-amber-500 text-white uppercase tracking-widest">
                            SISA {{ product.stock }}
                        </div>
                    </div>
