...
onMounted(async () => {
    try {
        ...
        if (
            storeData.value.payment_type === "midtrans" &&
            storeData.value.midtrans_client_key
        ) {
            if (!document.getElementById("midtrans-script")) {
                // Cek environment dari .env
                const midtransEnv =
                    import.meta.env.VITE_MIDTRANS_ENV || "sandbox";
                const snapUrl =
...
const handleInitialCheckout = async () => {
    ...
    if (
        ...
        Swal.fire({
            icon: "error",
            title: "Uang Kurang!",
            text: `Kurang Rp ${(totalBelanja.value - payAmount.value).toLocaleString("id-ID")}`,
        });
        return;
    }
    ...
    if (paymentMethod.value === "QRIS") {
        ...
        if (storeData.value?.payment_type === "midtrans") {
            if (typeof window.snap === "undefined") {
                Swal.fire(
                    "Sistem Loading",
                    "Script Midtrans belum siap, pastikan Client Key terisi di pengaturan Toko.",
                    "warning",
                );
                return;
            }
            ...
            try {
                ...
                Swal.fire(
                    "Error Backend",
                    "Gagal memanggil API Midtrans dari Golang.",
                    "error",
                );
                isProcessingCheckout.value = false;
            }
...
const triggerCheckoutFlow = () => {
    ...
    const hasWaToken =
        storeData.value?.wa_token && storeData.value.wa_token !== "";
...
<template>
    <div
        class="h-[100dvh] flex flex-col bg-slate-100 font-sans overflow-hidden print:h-auto print:bg-white print:overflow-visible print:block"
    >
        <PosHeader
            class="print:hidden"
        ...
        <div
            class="flex-1 flex overflow-hidden p-2 md:p-4 pt-2 md:pt-4 gap-4 relative print:hidden"
        >
            <ProductCatalog
        ...
        <div
            v-if="cart.length > 0"
            class="lg:hidden fixed bottom-0 left-0 right-0 p-3 bg-white/90 backdrop-blur-sm border-t border-slate-200 z-40 shadow-md print:hidden"
        >
            <button
                @click="isMobileCartOpen = true"
                class="w-full bg-indigo-600 text-white p-3 rounded-xl flex justify-between items-center active:scale-95 transition-all"
            >
                <span
                    class="bg-white text-indigo-600 font-black px-3 py-1 rounded-lg text-xs"
        ...
        <div
            v-if="showHeldModal"
            class="fixed inset-0 bg-slate-900/80 flex items-center justify-center z-[150] p-4 backdrop-blur-sm print:hidden"
        >
            <div
                class="bg-white p-6 md:p-8 rounded-[32px] shadow-2xl w-full max-w-xl max-h-[80vh] flex flex-col"
            >
                <div class="flex justify-between items-center mb-6">
                    <h2
                        class="text-lg font-black text-slate-800 uppercase tracking-widest flex items-center gap-2"
                    >
                        Pesanan Tertunda
                    </h2>
                    <button
                        @click="showHeldModal = false"
                        class="p-2 bg-slate-100 hover:bg-rose-100 text-slate-400 rounded-xl"
                    >
                        ✕
                    </button>
                </div>
                <div class="flex-1 overflow-y-auto custom-scrollbar pr-2">
                    <div
                        ...
                        class="text-center py-10 text-slate-400 font-bold text-xs uppercase tracking-widest italic"
                    >
                    ...
                    <div
                        ...
                        :key="order.id"
                        class="p-4 bg-slate-50 border border-slate-200 rounded-2xl mb-3 flex justify-between items-center group"
                    >
                        <div>
                            <p
                                class="font-black text-sm text-slate-800 uppercase"
                            >
                                {{ order.customer }}
                            </p>
                            <p
                            ...
                            <p class="text-indigo-600 font-black mt-1 text-sm">
                                Rp {{ order.total.toLocaleString("id-ID") }}
                            </p>
                        </div>
                        <button
                            @click="resumeOrder(order)"
                            class="bg-indigo-100 text-indigo-600 hover:bg-indigo-600 hover:text-white px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all"
                        >
                            Lanjutkan
                        </button>
                    </div>
        ...
        <div
            v-if="showScanner"
            class="fixed inset-0 bg-slate-900/90 flex items-center justify-center z-[150] p-4 backdrop-blur-sm print:hidden"
        >
            <div
                class="bg-white rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden flex flex-col"
            >
                <div
                    class="p-6 border-b flex justify-between items-center bg-slate-50/50"
                >
                    <h2
                        class="text-lg font-black text-slate-800 uppercase tracking-widest"
                    >
                        Scan Barcode
                    </h2>
                    <button
                        @click="stopScanner"
                        class="p-2 rounded-xl bg-slate-100 text-slate-400"
                    >
                        ✕
                    </button>
                </div>
                <div class="p-6 bg-black">
                    <div
                        id="reader-kasir"
                        class="w-full rounded-2xl overflow-hidden border-2 border-slate-700"
                    ></div>
                </div>
            </div>
        </div>
        ...
        >
            <div
                class="bg-white p-6 md:p-8 rounded-[32px] shadow-2xl w-full max-w-sm text-center flex flex-col border-t-8 border-indigo-600"
            >
                <h2
                    class="text-xl font-black text-slate-900 uppercase tracking-widest mb-1"
                >
                    Bayar via QRIS
                </h2>
                <div
                    class="bg-white p-3 rounded-2xl border border-slate-300 w-full mb-4 flex flex-col justify-center items-center min-h-[200px]"
                >
                    <img
                        v-if="storeData?.qris_image"
                    ...
                    <div
                        v-else
                        class="text-slate-400 text-xs font-bold p-10 text-center aspect-square flex items-center justify-center"
                    >
                        QRIS Belum Di-Setup.<br />Silakan upload di Pengaturan
                        Toko.
                    </div>

                    <p
                        v-if="storeData?.qris_name"
                        class="mt-3 font-bold text-[10px] text-slate-500 uppercase tracking-widest border-t border-slate-100 pt-2 w-full"
                    >
                        {{ storeData.qris_name }}
                    </p>
                </div>
                ...
                <div
                    class="bg-indigo-50 text-indigo-900 p-4 rounded-2xl mb-6 font-black text-2xl"
                >
                ...
                <div class="flex gap-3">
                    <button
                        ...
                        class="flex-1 bg-slate-100 py-4 rounded-xl font-black text-[10px] uppercase text-slate-500"
                    >
                        Batal
                    </button>
                    <button
                        @click="triggerCheckoutFlow"
                        :disabled="isProcessingCheckout"
                        class="flex-1 bg-indigo-600 py-4 rounded-xl font-black text-[10px] uppercase text-white shadow-lg"
                    >
                        {{ isProcessingCheckout ? "Proses..." : "Lunas" }}
                    </button>
                </div>
            </div>
        </div>
        ...
        <ReceiptModal
            ...
            :cashierName="
                currentUser?.name ? currentUser.name.split(' ')[0] : 'KASIR'
            "
