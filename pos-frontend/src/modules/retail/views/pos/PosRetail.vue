  ...
  try {
    ...
    if (role === "owner") {
      ...
      if (storeData.value.subscription_status !== "active") {
        ...
        const endDate = new Date(storeData.value.subscription_end), today = new Date();
...
const {
  currentUser, currentSession, currentTime, products, isLoadingProducts, cart, heldOrders, showHeldModal, payAmount, paymentMethod, showReceipt, showQrisModal, lastTransaction, showReceiptClosing, lastClosingData, isMobileCartOpen, searchQuery, searchInput, showScanner, pecahan, filteredProducts, subTotalBelanja, nilaiPajak, totalBelanja, kembalian, totalUangFisik, showClosingModal, isProcessingCheckout, storeLogo,
...
<template>
  <div class="h-[100dvh] flex flex-col bg-slate-100 font-sans overflow-hidden print:h-auto print:bg-white print:overflow-visible print:block">
    ...
    <div class="print:hidden flex-1 flex overflow-hidden p-2 md:p-4 pt-2 md:pt-4 gap-4 relative">
      ...
      <CartSidebar v-model:noHpPelanggan="noHpPelanggan" v-model:isMobileCartOpen="isMobileCartOpen" :cart="cart" :heldOrders="heldOrders" :paymentMethod="paymentMethod" :subTotal="subTotalBelanja" :pajak="nilaiPajak" :totalBelanja="totalBelanja" :payAmount="payAmount" :kembalian="kembalian" :isTaxActive="storeData?.is_tax_active" :pajakPersen="storeData?.pajak_percent" :isProcessingCheckout="isProcessingCheckout" @show-held="showHeldModal = true" @hold-order="holdTransaction" @clear-cart="clearCart" @decrease-qty="decreaseQty" @increase-qty="increaseQty" @validate-qty="validateQty" @set-payment="setPaymentMethod" @format-rupiah="formatInputRupiah" @set-nominal="payAmount += $event" @checkout="handleInitialCheckout" @toggle-uom="toggleUom" />
    </div>
    ...
    <div v-if="showQrisModal" class="fixed inset-0 bg-slate-900/80 flex items-center justify-center z-[150] p-4 backdrop-blur-sm print:hidden">
      <div class="bg-white p-6 md:p-8 rounded-[32px] shadow-2xl w-full max-w-sm text-center flex flex-col border-t-8 border-indigo-600">
        ...
        <div class="bg-white p-3 rounded-2xl border border-slate-300 w-full mb-4 flex flex-col justify-center items-center min-h-[200px]">
          ...
          <div v-else class="text-slate-400 text-xs font-bold p-10 text-center aspect-square flex items-center justify-center">QRIS Belum Di-Setup.<br />Silakan upload di Pengaturan Toko.</div>
