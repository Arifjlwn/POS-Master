...
const {
	isLoading,
	tanggalDipilih, // Berhubungan ke composable (format YYYY-MM-DD atau YYYY-MM)
	searchQuery,
	showReceipt,
	selectedTrx,
	filteredRiwayat,
...
// 📅 STATE UNTUK FILTER TANGGAL (HARIAN vs BULANAN)
const dateSearchType = ref('daily');
const bulanDipilih = ref(new Date().toISOString().substring(0, 7)); // Default: YYYY-MM bulan ini

// Watcher untuk sinkronisasi tipe pencarian ke composable backend
watch(
	[dateSearchType, bulanDipilih],
	() => {
		if (dateSearchType.value === 'monthly') {
			tanggalDipilih.value = bulanDipilih.value;
		} else {
			tanggalDipilih.value = new Date().toISOString().substring(0, 10);
		}
	},
	{ immediate: true }
);

// 🚀 PARSING AMAN UNTUK NAMA KASIR
const getCashierName = (item) => {
	const userObj = item?.User || item?.user;
	return userObj?.name || 'KASIR';
};

// 🚀 PARSING AMAN UNTUK FORMAT JAM (Mencegah 'Invalid Date')
...
// 🚀 PARSING AMAN UNTUK FORMAT TANGGAL + JAM LENGKAP (Buka & Tutup Shift)
const formatTanggalJam = (dateString) => {
	if (!dateString || dateString.startsWith('0001')) return '--:--';
	try {
		const d = new Date(dateString);
		if (isNaN(d.getTime())) return '--:--';
...
// 🚀 FILTER SEARCH LIVE UNTUK TAB CLOSING
...
<template>
	<Sidebar class="print:bg-white print:h-auto">
		<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans bg-[#f8fafc] min-h-screen print:hidden">
			<div class="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 rounded-[32px] p-6 md:p-10 mb-6 text-white shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between relative overflow-hidden gap-6">
				<div class="z-10 text-center lg:text-left">
					<h1 class="text-3xl md:text-4xl font-black tracking-tighter mb-1 uppercase italic">
						Daily
						<span class="text-blue-400">Journal</span>
					</h1>
					<p class="text-slate-300 font-bold text-[10px] md:text-xs uppercase tracking-[0.2em]">Audit, Reprint & Track Store Sales</p>
				</div>

				<div class="z-10 flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
					<div class="flex bg-white/10 backdrop-blur-md p-1 rounded-xl border border-white/10 w-full sm:w-auto">
						<button @click="dateSearchType = 'daily'" :class="dateSearchType === 'daily' ? 'bg-blue-600 text-white font-black' : 'text-slate-300 font-bold hover:text-white'" class="flex-1 sm:flex-initial px-4 py-2 rounded-lg text-xs uppercase tracking-wider transition-all">Harian</button>
						<button @click="dateSearchType = 'monthly'" :class="dateSearchType === 'monthly' ? 'bg-blue-600 text-white font-black' : 'text-slate-300 font-bold hover:text-white'" class="flex-1 sm:flex-initial px-4 py-2 rounded-lg text-xs uppercase tracking-wider transition-all">Bulanan</button>
					</div>

					<div class="flex items-center justify-center gap-3 bg-white/10 backdrop-blur-md p-3.5 rounded-2xl border border-white/20 shadow-inner w-full sm:w-auto min-w-[200px]">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-blue-300 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
						...
						<input v-if="dateSearchType === 'daily'" type="date" v-model="tanggalDipilih" class="bg-transparent border-none text-sm md:text-base font-black text-white focus:ring-0 cursor-pointer outline-none uppercase tracking-tighter w-full [color-scheme:dark]" />

						<input v-else type="month" v-model="bulanDipilih" class="bg-transparent border-none text-sm md:text-base font-black text-white focus:ring-0 cursor-pointer outline-none uppercase tracking-tighter w-full [color-scheme:dark]" />
					</div>
				</div>
			</div>
					...
					<div v-for="trx in filteredRiwayat" :key="trx.id" class="bg-white p-5 rounded-[24px] shadow-sm border border-slate-100 flex flex-col gap-3 relative overflow-hidden">
						...
						<div class="flex justify-between items-start">
							<div>
								...
								<div class="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{{ formatJam(trx.created_at) }} • OPR: {{ getCashierName(trx).split(' ')[0] }}</div>
							</div>
			...
			<div v-if="activeTab === 'closing'">
						...
						<div class="flex justify-between items-start">
							<div>
								...
								<div class="text-[10px] font-bold text-slate-400 uppercase mt-0.5">
									BUKA: {{ formatTanggalJam(close.opened_at) }}
									<br />
									TUTUP: {{ formatTanggalJam(close.closed_at) }}
								</div>
							</div>
				...
				<div v-if="filteredRiwayatClosing && filteredRiwayatClosing.length > 0" class="hidden md:block w-full bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden transition-all">
					<table class="w-full text-left border-collapse whitespace-nowrap">
						<thead>
							<tr class="bg-slate-50 border-b border-slate-100">
								<th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Buka Shift</th>
								<th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tutup Shift</th>
								<th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama (Kasir)</th>
						...
						<tbody class="divide-y divide-slate-50">
							<tr v-for="close in filteredRiwayatClosing" :key="close.id" class="hover:bg-blue-50/40 transition-all group">
								<td class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">
									{{ formatTanggalJam(close.opened_at) }}
								</td>

								<td class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">
									{{ formatTanggalJam(close.closed_at) }}
								</td>
								...
								<td class="px-6 py-4 text-right font-black text-slate-800 text-sm">Rp{{ formatRupiah(close.total_expected) }}</td>

								<td class="px-6 py-4 text-right font-black text-slate-800 text-sm">Rp{{ formatRupiah(close.total_actual) }}</td>

								<td class="px-6 py-4 text-right font-black text-sm" :class="close.selisih < 0 ? 'text-rose-500' : close.selisih > 0 ? 'text-amber-500' : 'text-emerald-500'">Rp{{ formatRupiah(close.selisih) }}</td>
								<td class="px-6 py-4 text-center">
	...
	<ReceiptModal :show="showReceipt" :invoiceData="selectedTrx" :storeData="selectedTrx?.Store || selectedTrx?.store || currentSession?.store || currentSession?.Store" :cashierName="selectedTrx ? getCashierName(selectedTrx).split(' ')[0] : 'KASIR'" :stationNumber="'01'" @close="showReceipt = false" />
