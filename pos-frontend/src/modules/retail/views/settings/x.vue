...
onMounted(async () => {
	try {
		...
		isDataLoaded.value = true; // 2. Tandai data sudah ditarik
	}
});
const saveSettings = async () => {
	...
	try {
		...
		Swal.fire({ icon: 'success', title: 'Tersimpan', text: 'Token berhasil diperbarui!', timer: 1500 });
	} catch (error) {
...
const testConnection = async () => {
	...
	try {
		const response = await api.post('/retail/whatsapp/test', {
		...
		if (response.data.status) {
			...
			Swal.fire('Gagal', response.data.reason || 'Cek token!', 'error');
		}
	} catch (error) {
