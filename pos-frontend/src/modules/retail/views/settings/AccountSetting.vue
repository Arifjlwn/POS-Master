	...
	try {
		...
		window.snap.pay(snapToken, {
			onSuccess: async function (result) {
				...
				Swal.close();
				Swal.fire('Berhasil!', 'Pembayaran sukses! Kuota Mesin Kasir Anda telah bertambah.', 'success').then(() => {
								...
								<div class="max-w-md">
									<div class="bg-indigo-50/50 border-2 border-indigo-100 p-6 rounded-[24px] flex flex-col justify-between transition-all hover:border-indigo-300">
										<div>
											...
											<p class="text-[11px] font-bold text-indigo-600 mt-2 leading-relaxed">Menentukan jumlah perangkat (HP/Tablet/PC) yang bisa membuka laci kasir secara bersamaan.</p>
										...
										<button @click="beliLisensiTambahan" :disabled="loadingPayment" class="mt-6 w-full bg-indigo-600 hover:bg-slate-900 text-white py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-50">
				...
				<div class="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						<div class="bg-white rounded-[24px] p-6 border border-slate-200 flex flex-col relative">
							...
							<button v-if="subPlan?.toLowerCase() === 'trial'" disabled class="w-full py-3.5 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest cursor-not-allowed">Paket Saat Ini</button>

							<button v-else disabled class="w-full py-3.5 bg-slate-50 border border-slate-200 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest cursor-not-allowed opacity-70">Khusus Pengguna Baru</button>
						</div>
						...
						<div class="bg-white rounded-[24px] p-6 border-2 border-indigo-500 shadow-xl relative flex flex-col transform lg:-translate-y-4">
							...
							<button v-if="subPlan?.toLowerCase() === 'pro'" disabled class="w-full py-3.5 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest cursor-not-allowed">Paket Saat Ini</button>

							<button v-else @click="handleUpgrade('Pro')" class="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 transition-all active:scale-95">Upgrade Pro</button>
						</div>
