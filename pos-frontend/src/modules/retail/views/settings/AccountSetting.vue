...
const isExpired = ref(false); // 🚀 STATE DEAD-END

onMounted(async () => {
    // ==============================================================
    // 🚀 SUNTIK SCRIPT MIDTRANS DINAMIS (BACA .ENV)
    // ==============================================================
    ...
    try {
        // Cukup tarik data status langganan dari DB untuk validasi halaman ter-lock atau tidak
        ...
        if (storeData.subscription_end) {
            ...
            // Mengunci layar hitam HANYA jika statusnya emang benar-benar inactive/expired di DB
            if (diffDays <= 0 || subStatus.value !== 'active') {
                isExpired.value = true; 
            }
        } else {
            subEnd.value = 'Tidak Terbatas';
            if (subStatus.value !== 'active') isExpired.value = true;
        }
    ...
    try {
        ...
        window.snap.pay(snapToken, {
            onSuccess: function(result){
                ...
                // 🚀 SUNTIK MEMORI BARU KE BROWSER BIAR GAK PERLU LOGOUT!
                localStorage.setItem('subscriptionPlan', planName.toLowerCase());
                
                // 🚀 KASIH LOADING 3 DETIK BIAR WEBHOOK GOLANG KELAR KERJA DULU
                Swal.fire({
                    title: 'Pembayaran Sukses!', 
                    text: 'Sistem sedang memverifikasi dan membuka gembok toko Anda...', 
                    icon: 'success',
                    timer: 2000, // Tahan 2 detik!
                    showConfirmButton: false,
                    allowOutsideClick: false
                }).then(() => {
                    // Habis 2 detik, arahin langsung ke Dashboard!
                    window.location.href = '/retail/account?tab=billing'; 
                });
            },
            ...
            onClose: function(){
                /* Ketika user batal bayar / close snap, baru kita munculin gembok aktif berakhir */
                isExpired.value = true;
    ...
    <Sidebar>
        ...
        <div v-if="isExpired" class="fixed inset-0 z-[9999] bg-slate-900 flex flex-col items-center justify-center p-4 md:p-8 overflow-y-auto">
            <div class="text-center mt-32 md:mt-10 mb-10 max-w-2xl mx-auto animate-pulse">
                ...
                <p class="text-slate-400 font-bold tracking-widest text-xs md:text-sm uppercase">Operasional sistem POS dihentikan sementara.<br>Silakan perpanjang paket untuk membuka kembali akses toko Anda.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full mb-20">
                <div class="bg-slate-800 rounded-[24px] p-6 border border-slate-700 flex flex-col text-center">
                    ...
                    <div class="flex items-baseline justify-center gap-1 mb-6">
                        <span class="text-sm font-black text-slate-400">Rp</span><span class="text-5xl font-black text-white">49<span class="text-lg text-slate-500">k</span></span>
                    </div>
                    <button @click="handleUpgrade('Basic', 49000)" class="w-full py-4 bg-sky-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-sky-400 transition-all active:scale-95">Perpanjang Basic</button>
                </div>
                ...
                <div class="bg-slate-800 rounded-[24px] p-6 border-2 border-indigo-500 shadow-xl shadow-indigo-500/20 relative flex flex-col text-center transform md:-translate-y-4">
                    ...
                    <div class="flex items-baseline justify-center gap-1 mb-6">
                        <span class="text-sm font-black text-slate-400">Rp</span><span class="text-5xl font-black text-white">149<span class="text-lg text-slate-500">k</span></span>
                    </div>
                    <button @click="handleUpgrade('Pro', 149000)" class="w-full py-4 bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-indigo-400 transition-all active:scale-95">Perpanjang Pro</button>
                </div>
                ...
                <div class="bg-slate-800 rounded-[24px] p-6 border border-slate-700 flex flex-col text-center">
                    ...
                    <div class="flex items-baseline justify-center gap-1 mb-6">
                        <span class="text-sm font-black text-slate-400">Rp</span><span class="text-5xl font-black text-white">299<span class="text-lg text-slate-500">k</span></span>
                    </div>
                    <button @click="handleUpgrade('Premium', 299000)" class="w-full py-4 bg-amber-500 text-slate-900 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-amber-400 transition-all active:scale-95">Perpanjang Premium</button>
                </div>
            </div>
        </div>
                ...
                <div class="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="bg-slate-900 rounded-[24px] p-6 border border-slate-700 flex flex-col">
                            ...
                            <button v-if="['Premium', 'premium'].includes(subPlan?.toLowerCase())" disabled class="w-full py-3.5 bg-slate-800 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest cursor-not-allowed">
                                Paket Saat Ini
                            </button>
