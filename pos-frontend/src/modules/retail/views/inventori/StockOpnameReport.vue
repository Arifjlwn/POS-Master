...
const { reports, isLoading, selectedDetail, isOwner, isApproving, showDetail, calculateLoss, formatDate, approveAudit } = useStockOpnameReport();
</script>
		...
		<div id="stock-opname-view" class="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto font-sans bg-[#f8fafc] min-h-screen">
			<div class="no-print bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 rounded-[40px] p-8 md:p-10 mb-6 text-white shadow-2xl flex flex-col md:flex-row items-center justify-between relative overflow-hidden border border-white/10 select-none">
				<div class="z-10 text-center md:text-left">
					<h1 class="text-3xl md:text-4xl font-black tracking-tighter mb-2 uppercase italic leading-none">
						Audit &
						<span class="text-indigo-400">Claims</span>
					</h1>
			...
			<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-6">
				<div class="lg:col-span-4 no-print space-y-6">
					...
					<ReportList v-else :reports="reports" :selectedDetail="selectedDetail" :formatDate="formatDate" :calculateLoss="calculateLoss" @select="showDetail" />
				</div>

				<div class="lg:col-span-8">
					<ReportDetail v-if="selectedDetail && !isLoading" :detail="selectedDetail" :isOwner="isOwner" :isApproving="isApproving" :formatDate="formatDate" :calculateLoss="calculateLoss" @approve="(id, type, file) => approveAudit(id, type, file)" />
...
/* ==========================================
   ...
   ========================================== */
@media print {
	/* Gembok total sirkulasi shadow DOM sidebar pas print view dipicu  */
	:deep(aside),
	:deep(nav),
	:deep(header),
