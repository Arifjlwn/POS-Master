...
const { reportData, isLoading, storeName, startDate, endDate, lineChartCanvas, pieChartCanvas, profitMargin, strukPerHari, formatRupiah, setQuickFilter, fetchData } = useDashboard();
...
// 🚀 SMART FALLBACK: Auto-Kalkulasi Laba Bersih di sisi Frontend
const labaKotor = computed(() => reportData.value?.summary?.total_laba_gross ?? reportData.value?.summary?.total_laba ?? 0);
const labaBersih = computed(() => {
	// Kalau backend udah update pake total_laba_netto, pake itu
	if (reportData.value?.summary?.total_laba_netto !== undefined) {
		return reportData.value.summary.total_laba_netto;
	}
	// Kalau belom, hitung manual: Laba Kotor - Rugi Retur - Rugi SO
	return labaKotor.value - (reportData.value?.summary?.total_retur_loss || 0) - (reportData.value?.summary?.net_so_loss || 0);
});
// 🔴 Detektor Toko Boncos
const isBoncos = computed(() => labaBersih.value < 0);
</script>
...
<template>
	<Sidebar>
		<div class="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto font-sans bg-[#F8FAFC] min-h-screen">
			...
			<div v-else>
				<!-- 🚀 GRID DIPERBAIKI: Card 7 dibikin col-span-2 biar layoutnya genap simetris  -->
				<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
					...
					<div class="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-blue-300 transition-all relative overflow-hidden">
						<div class="absolute right-0 top-0 w-20 h-20 bg-gradient-to-br from-blue-50 to-transparent opacity-60 rounded-bl-[100%]"></div>
						<div class="flex justify-between items-start mb-4 relative z-10">
							<p class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Struk & STD</p>
						...
						<div class="relative z-10 flex items-end justify-between">
							<div>
								<p class="text-2xl font-black text-slate-900 tracking-tight">
									{{ formatNumber(reportData?.summary?.jumlah_transaksi || 0) }}
									<span class="text-[12px] font-bold text-slate-400">Struk</span>
								</p>
								<p class="text-[10px] font-bold text-slate-400 mt-1">Total Kunjungan Transaksi</p>
							</div>
							<div class="flex items-center gap-1 bg-blue-50 text-blue-700 px-2.5 py-1 rounded-lg text-[10px] font-black tracking-widest border border-blue-100 uppercase">
								<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" /></svg>
								{{ strukPerHari }} / Hari
							</div>
						</div>
					</div>
					...
					<div class="bg-slate-900 p-6 rounded-[24px] shadow-lg flex flex-col justify-between group relative overflow-hidden">
						...
						<div class="flex justify-between items-start mb-4 relative z-10">
							<p class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Avg. Per Struk (APC)</p>
					...
					<!-- 🔴 SMART LABA CARD: Warna dinamis mengikuti keuntungan/kerugian -->
					<div class="bg-white p-6 rounded-[24px] border shadow-sm flex flex-col justify-between group transition-all relative overflow-hidden" :class="isBoncos ? 'border-rose-200 hover:border-rose-400' : 'border-slate-200 hover:border-emerald-300'">
						<div class="absolute right-0 top-0 w-20 h-20 bg-gradient-to-br opacity-60 rounded-bl-[100%]" :class="isBoncos ? 'from-rose-50 to-transparent' : 'from-emerald-50 to-transparent'"></div>
						<div class="flex justify-between items-start mb-4 relative z-10">
							<p class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Laba Bersih (Netto)</p>
							<div class="w-8 h-8 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform" :class="isBoncos ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'">
								<svg v-if="!isBoncos" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>
								<svg v-else xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
							</div>
						</div>
						<div class="relative z-10 flex items-end justify-between">
							<div>
								<p class="text-2xl font-black tracking-tight" :class="isBoncos ? 'text-rose-600' : 'text-emerald-600'">{{ formatRupiah(labaBersih) }}</p>
								<p class="text-[10px] font-bold text-slate-400 mt-1">
									Laba Kotor:
									<span class="text-slate-600">{{ formatRupiah(labaKotor) }}</span>
								</p>
							</div>
							<div class="px-2 py-1 rounded-lg text-[9px] font-black tracking-widest border" :class="isBoncos ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'">
								<span v-if="isBoncos">RUGI / BONCOS</span>
								<span v-else>{{ profitMargin }}% MARGIN</span>
							</div>
						</div>
					</div>
					...
					<div class="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-amber-300 transition-all">
						...
						<div>
							<p class="text-2xl font-black text-slate-900 tracking-tight">
								{{ formatNumber(reportData?.summary?.total_produk_terjual) }}
								<span class="text-[12px] font-bold text-slate-400">Total</span>
							</p>
					...
					<div class="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-rose-300 transition-all relative overflow-hidden">
						...
						<div class="relative z-10">
							...
							<p class="text-[10px] font-bold text-slate-400 mt-1">
								Total:
								<span class="text-rose-500">{{ formatNumber(reportData?.summary?.total_retur_qty) }}</span>
								Dibuang/Rusak
							</p>
						</div>
					</div>

					<div class="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-purple-300 transition-all relative overflow-hidden sm:col-span-2 xl:col-span-2">
						...
						<div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
							<div class="mb-3 sm:mb-0">
								...
								<p class="text-[10px] font-bold text-slate-400 mt-0.5">
									Total Akhir:
									<span class="text-purple-500">{{ formatNumber(reportData?.summary?.net_so_qty) }}</span>
									Item Hilang
								</p>
							</div>
							<div class="flex-1 max-w-[200px] flex flex-col gap-1.5 sm:pt-0 pt-3 sm:border-t-0 border-t border-slate-100 sm:border-l sm:pl-4">
				...
				<!-- LINE CHART GRAPH & PIE COMPOSITION SECTION -->
				<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
						...
						<div class="flex items-center justify-between mb-6">
							...
							<div class="flex gap-3">
								<div class="flex items-center gap-1.5">
									<span class="w-2.5 h-2.5 rounded-sm bg-[#4f46e5]"></span>
									<span class="text-[9px] font-black text-slate-600 uppercase">Omzet</span>
								</div>
								<div class="flex items-center gap-1.5">
									<span class="w-2.5 h-2.5 rounded-sm border-2 border-dashed border-[#10b981]"></span>
									<span class="text-[9px] font-black text-slate-600 uppercase">Laba</span>
								</div>
								<div class="flex items-center gap-1.5">
									<span class="w-2.5 h-2.5 rounded-sm bg-[#e11d48]"></span>
									<span class="text-[9px] font-black text-slate-600 uppercase">Retur</span>
								</div>
							</div>
						</div>
						<div class="flex-1 min-h-[250px] w-full relative"><canvas ref="lineChartCanvas"></canvas></div>
					</div>
					...
					<div class="bg-white rounded-[24px] p-6 border border-slate-200 shadow-sm flex flex-col">
						...
						<div v-if="!reportData?.best_sellers || reportData.best_sellers.length === 0" class="flex-1 flex flex-col items-center justify-center text-slate-300 py-10">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mb-2 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
								<circle cx="12" cy="12" r="10" />
				...
				<div class="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden">
					<div class="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
						<h3 class="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-amber-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
							...
							<tbody class="divide-y divide-slate-50">
								<tr v-for="(item, index) in reportData.best_sellers" :key="index" class="hover:bg-slate-50/50 transition-colors">
									<td class="px-6 py-3 text-center">
										<span class="inline-flex items-center justify-center w-6 h-6 rounded-md text-[10px] font-black" :class="index === 0 ? 'bg-amber-100 text-amber-600' : index === 1 ? 'bg-slate-200 text-slate-600' : index === 2 ? 'bg-orange-100 text-orange-700' : 'text-slate-400'">#{{ index + 1 }}</span>
									</td>
									<td class="px-6 py-3">
									...
									<td class="px-6 py-3 text-center">
										<span class="font-black text-slate-700 text-xs bg-slate-100 px-2.5 py-1 rounded-md">{{ formatNumber(item.qty_terjual) }} {{ item.satuan_dasar || 'Pcs' }}</span>
									</td>
