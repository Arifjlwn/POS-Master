...
const labaBersih = computed(() => {
    // Kalau backend udah update pake total_laba_netto, pake itu bray
...
<template>
  <Sidebar>
    <div class="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto font-sans bg-[#F8FAFC] min-h-screen">
      ...
      <div v-else>
        <!-- 🚀 GRID DIPERBAIKI: Card 7 dibikin col-span-2 biar layoutnya genap simetris bray -->
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
          ...
          <div class="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-blue-300 transition-all relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-end justify-between">
              <div>
                <p class="text-2xl font-black text-slate-900 tracking-tight">{{ formatNumber(reportData?.summary?.jumlah_transaksi || 0) }} <span class="text-[12px] font-bold text-slate-400">Struk</span></p>
          ...
          <div class="bg-white p-6 rounded-[24px] border shadow-sm flex flex-col justify-between group transition-all relative overflow-hidden" :class="isBoncos ? 'border-rose-200 hover:border-rose-400' : 'border-slate-200 hover:border-emerald-300'">
            ...
            <div class="relative z-10 flex items-end justify-between">
              <div>
                ...
                <p class="text-[10px] font-bold text-slate-400 mt-1">Laba Kotor: <span class="text-slate-600">{{ formatRupiah(labaKotor) }}</span></p>
              </div>
            ...
            <div v-else class="flex-1 flex flex-col justify-center mt-6">
              ...
              <div class="space-y-1.5 mt-auto max-h-[110px] overflow-y-auto custom-scrollbar">
                <div v-for="(item, index) in reportData.best_sellers.slice(0, 5)" :key="index" class="flex items-center justify-between text-[10px] font-bold text-slate-600 py-0.5 border-b border-slate-50">
                  <div class="flex items-center gap-2 truncate pr-2"><span class="w-2 h-2 rounded-full shrink-0" :style="{ backgroundColor: ['#4f46e5', '#3b82f6', '#0ea5e9', '#10b981', '#f59e0b'][index] }"></span><span class="truncate uppercase">{{ item.nama_produk }}</span></div>
            ...
            <table class="w-full text-left whitespace-nowrap">
              ...
              <tbody class="divide-y divide-slate-50">
                <tr v-for="(item, index) in reportData.best_sellers" :key="index" class="hover:bg-slate-50/50 transition-colors">
                  <td class="px-6 py-3 text-center"><span class="inline-flex items-center justify-center w-6 h-6 rounded-md text-[10px] font-black" :class="index === 0 ? 'bg-amber-100 text-amber-600' : index === 1 ? 'bg-slate-200 text-slate-600' : index === 2 ? 'bg-orange-100 text-orange-700' : 'text-slate-400'">#{{ index + 1 }}</span></td>
                  <td class="px-6 py-3">
                  ...
                  <td class="px-6 py-3 text-center"><span class="font-black text-slate-700 text-xs bg-slate-100 px-2.5 py-1 rounded-md">{{ formatNumber(item.qty_terjual) }} {{ item.satuan_dasar || 'Pcs' }}</span></td>
                  <td class="px-6 py-3 text-right"><span class="font-black text-indigo-600 text-sm">{{ formatRupiah(item.total_omzet) }}</span></td>
                </tr>
              </tbody>
            </table>
          </div>
