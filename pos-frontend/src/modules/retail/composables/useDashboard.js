...
export function useDashboard() {
    ...
    // 🚀 FIX PROFIT MARGIN: Hitung margin murni berdasarkan Laba Netto baru dari backend Go bray!
    const profitMargin = computed(() => {
        ...
        const totalLabaNetto = reportData.value.summary.total_laba_netto ?? 
                               (reportData.value.summary.total_laba_gross ?? reportData.value.summary.total_laba ?? 0);

        if (totalOmzet === 0) return 0;
        return ((totalLabaNetto / totalOmzet) * 100).toFixed(1);
    });

    // 🚀 FIX STRUK PER HARI: Langsung baca kalkulasi solid dari level database via Backend payload bray!
    ...
    const renderLineChart = (grafikData = []) => {
        ...
        // SAFE MEMORY DESTRUCTION: Hancurkan instance lama biar gak stack dan flicker pas di-hover bray!
    ...
    const fetchData = async () => {
        try {
            ...
            // Simpan data array terstruktur murni bray
            ...
            errorMessage.value = 'Gagal memuat data laporan. Silakan coba lagi bray.';
            isLoading.value = false;
        }
    };
