...
export function usePos() {
  ...
  const storeSettings = ref({
    payment_type: 'qris_static', // Default
    qris_image: '',
    qris_name: '',
    is_tax_active: false,
    pajak_persen: 0,
  ...
  const startScanner = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        Swal.fire(
          'Oops!',
          'Gagal menyalakan kamera. Pastikan izin kamera aktif.',
          'error'
        );
        stopScanner();
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      products.value = response.data.map((p) => ({
        ...
        harga_jual_tengah:
          p.harga_jual_tengah || p.harga_jual * (p.isi_tengah_ke_dasar || 1),
      }));
    } catch (error) {
  ...
  const handleBarcodeScan = () => {
    ...
    const exactMatch = products.value.find(
      (p) => p.sku && String(p.sku).toLowerCase() === query
    );
  ...
  const addToCart = (product) => {
    ...
    const existingItem = cart.value.find(
      (item) => item.id === product.id && item.selected_uom === defaultUom
    );

    if (existingItem) {
      if (
        (existingItem.qty + 1) * existingItem.uom_multiplier <=
        product.stock
      ) {
        existingItem.qty++;
      } else {
      ...
      if (item.selected_uom === item.satuan_dasar) {
        if (item.qty * item.isi_tengah > prodMaster.stock) {
          Swal.fire({
            icon: 'error',
            title: 'Stok Kurang',
            text: `Stok tak cukup untuk ${item.satuan_tengah}`,
          });
          return;
        }
        ...
        if (item.qty * item.isi_besar > prodMaster.stock) {
          Swal.fire({
            icon: 'error',
            title: 'Stok Kurang',
            text: `Stok tak cukup untuk ${item.satuan_besar}`,
          });
  ...
  const decreaseQty = (item) => {
    const existingItem = cart.value.find(
      (i) => i.id === item.id && i.selected_uom === item.selected_uom
    );
    if (existingItem) {
  ...
  const validateQty = (item) => {
    ...
    if (
      existingItem &&
      prodMaster &&
      existingItem.qty * existingItem.uom_multiplier > prodMaster.stock
    ) {
      Swal.fire({
        icon: 'warning',
        title: 'Overstock!',
        text: 'Kuantitas dikembalikan ke batas stok',
      });
      existingItem.qty = Math.floor(
        prodMaster.stock / existingItem.uom_multiplier
      );
    }
  };
  ...
  const nilaiPajak = computed(() => {
    if (!storeSettings.value || !storeSettings.value.is_tax_active) return 0;
    return (storeSettings.value.pajak_persen / 100) * subTotalBelanja.value;
  });
  const totalBelanja = computed(
    () => Math.round((subTotalBelanja.value + nilaiPajak.value) / 100) * 100
  );
  ...
  const formatInputRupiah = (event) => {
    ...
    event.target.value =
      payAmount.value === 0 ? '' : payAmount.value.toLocaleString('id-ID');
  };

  // --- 🚀 PROSES CHECKOUT (QRIS vs CASH vs MIDTRANS) ---
  const isProcessingCheckout = ref(false);

  // Ini fungsi final yang nyimpen ke database Golang setelah dibayar lunas
  const executeCheckout = async () => {
    ...
    try {
      ...
      Swal.fire(
        'Gagal!',
        error.response?.data?.error || 'Koneksi terputus',
        'error'
      );
    } finally {
  ...
  // --- 🚀 PROSES CHECKOUT (VERSI DEBUGGING CEPAT) ---
  const processCheckout = async () => {
    ...
    if (paymentMethod.value === 'QRIS') {
      ...
      if (tipePayment === 'midtrans') {
        // 1. CEK SCRIPT SNAP: Apakah Client Key beneran ada dan script berhasil disuntik?
        if (typeof window.snap === 'undefined') {
          Swal.fire({
            icon: 'error',
            title: 'Script Midtrans Gagal!',
            text: `Client Key kosong atau script belum ke-load. Pastikan Client Key terisi di Pengaturan Toko!`,
          });
          return;
        }
        ...
        try {
          // 2. TEMBAK GOLANG: Minta Token
          const payRes = await api.post('/retail/pos/midtrans-order', {
            total: totalBelanja.value,
          });

          // 3. CEK TOKEN: Golang ngasih token nggak?
          if (!payRes.data || !payRes.data.token) {
            Swal.fire(
              'Error Backend',
              'Golang tidak mengembalikan Token Snap!',
              'error'
            );
            isProcessingCheckout.value = false;
            return;
          ...
          Swal.fire({
            icon: 'error',
            title: 'API Golang Error!',
            text:
              error.response?.data?.error ||
              'Endpoint /retail/pos/midtrans-order gagal dipanggil.',
          });
          isProcessingCheckout.value = false;
        }
      } else {
        // JIKA TOKO BUKAN MIDTRANS
        Swal.fire({
          icon: 'warning',
          title: 'Bukan Midtrans!',
          text: `Tipe di Database saat ini adalah: "${tipePayment}". Makanya lari ke QRIS Statis!`,
        });
  ...
  const totalUangFisik = computed(() => {
    return (
      pecahan.value.p100k * 100000 +
      pecahan.value.p50k * 50000 +
      pecahan.value.p20k * 20000 +
      pecahan.value.p10k * 10000 +
      pecahan.value.p5k * 5000 +
      pecahan.value.p2k * 2000 +
      pecahan.value.p1k * 1000 +
      pecahan.value.p500 * 500 +
      pecahan.value.p200 * 200 +
      pecahan.value.p100 * 100 +
      pecahan.value.p50 * 50 +
      pecahan.value.p25 * 25
    );
  });
  ...
  onMounted(async () => {
    ...
    try {
      ...
      // 🚀 SUNTIK SCRIPT MIDTRANS DINAMIS (Hanya jika tipe midtrans)
      if (
        storeSettings.value.payment_type === 'midtrans' &&
        storeSettings.value.midtrans_client_key
      ) {
  ...
  return {
    ...
    holdTransaction,
    resumeOrder,
    storeSettings, // 🚀 Export storeSettings
    setPaymentMethod,
    executeCheckout,
    formatInputRupiah,
    processCheckout,
