...
export function usePos() {
  ...
  const validateQty = (item) => {
    ...
    if (existingItem && prodMaster && (existingItem.qty * existingItem.uom_multiplier) > prodMaster.stock) {
      ...
      existingItem.qty = Math.floor(prodMaster.stock / existingItem.uom_multiplier) || 1;
    }
  };
      ...
      if (tipePayment === 'midtrans') {
        ...
        try {
          ...
          window.snap.pay(payRes.data.token, {
            ...
            onPending: () => { Swal.fire('Menunggu', 'Menunggu pelanggan menyelesaikan transfer di aplikasi banking.', 'info'); isProcessingCheckout.value = false; },
            onError: () => { Swal.fire('Gagal', 'Sesi pembayaran QRIS ditolak oleh Bank / Merchant.', 'error'); isProcessingCheckout.value = false; },
