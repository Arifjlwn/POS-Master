...
export function useStoreSetting() {
    ...
    const saveSettings = async () => {
        ...
        Object.keys(form.value).forEach(key => {
            if (key !== 'logo_url' && key !== 'qris_image') {
                // Pastikan nilai boolean dikirim sebagai string agar bisa dibaca Golang
                const val = typeof form.value[key] === 'boolean' ? String(form.value[key]) : form.value[key];
    ...
    return {
        ...
        handleFileChange, removeLogo, removeQris, saveSettings
    };
}
