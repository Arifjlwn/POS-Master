...
export function usePenerimaanBarang() {
	...
	const hitungModalPerPcs = (item) => {
		const hargaTotalFaktur = Number(item.harga_beli_input) || 0;
		const totalQtyDasar = hitungTotalStok(item);

		// Hindari error Infinity / dibagi 0
		if (totalQtyDasar <= 0) return 0;

		// Sistem langsung ngebelah total uang keluar dibagi kuantitas fisik yang masuk
		return Math.round(hargaTotalFaktur / totalQtyDasar);
	};
