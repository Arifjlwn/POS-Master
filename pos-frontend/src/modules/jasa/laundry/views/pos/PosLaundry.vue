...
const isPrintModalOpen = computed(() => !!posEngine.printData.value);
</script>
			...
			<div class="p-4 md:p-6 shrink-0 bg-white shadow-sm z-10 flex flex-col gap-4 border-b border-slate-200">
				<div class="bg-slate-900 rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row justify-between sm:items-center text-white shadow-lg gap-4">
					<div class="flex items-center gap-3">
						<button @click="router.push('/laundry/status')" class="bg-rose-500/20 hover:bg-rose-600 text-rose-200 hover:text-white p-2 md:p-2.5 rounded-xl border border-rose-500/30 hover:border-rose-500 transition-all active:scale-95 flex items-center justify-center shrink-0 shadow-sm" title="Keluar dari Kasir">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
								<path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
							</svg>
						</button>

						<div class="bg-white/10 p-2 rounded-xl border border-white/5 hidden sm:block">
