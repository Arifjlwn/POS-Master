...
export function usePosLaundry() {
    ...
    // 🚀 ENGINE KALKULATOR TANGGAL OTOMATIS BRAY!
    const hitungEstimasiOtomatis = () => {
        if (cart.value.length === 0) {
            // Kalau keranjang kosong, balikin ke default
            resetEstimasiSelesai();
            return;
        }

        let maxDate = new Date(); // Mulai dari hari ini, detik ini
        
        cart.value.forEach(item => {
            const itemDate = new Date(); 
            const durasi = parseInt(item.estimasi_durasi) || 0;
            
            if (item.estimasi_satuan === 'Hari') {
                itemDate.setDate(itemDate.getDate() + durasi);
            } else if (item.estimasi_satuan === 'Jam') {
                itemDate.setHours(itemDate.getHours() + durasi);
            }
            
            // 🎯 Cari tanggal yang PALING LAMA (Paling max)
            if (itemDate > maxDate) {
                maxDate = itemDate;
            }
        });

        // 🎯 Tembak ke input kalender format YYYY-MM-DD
        estimasiSelesai.value = maxDate.toISOString().split('T')[0];
    };

    // 🚀 PANTULAN OTOMATIS: Tiap keranjang nambah/ngurang, hitung ulang!
    watch(() => cart.value, () => {
        hitungEstimasiOtomatis();
    }, { deep: true });

    // 🚀 ENGINE DEFAULT: Buat reset tanggal otomatis ke H+2 (Fallback kalau gak ada item)
    ...
    const addToCart = (service) => {
        if (cart.value.findIndex((item) => item.id === service.id) !== -1) {
            ...
            cart.value.push({
                ...
                harga_parfum: 0,
                // 🚀 FIXED: Injeksi data untuk kalkulasi estimasi selesai
                estimasi_durasi: service.estimasi_durasi || 1,
                estimasi_satuan: service.estimasi_satuan || 'Hari'
            });
        }
    };
    ...
    return {
        ...
        removeCartItem: (index) => {
            ...
            if (cart.value.length === 0) {
                isCartOpen.value = false;
                resetEstimasiSelesai(); // 🚀 Kalau keranjang dibersihkan manual, balikin ke default
            }
        },
        clearCart: () =>
            Swal.fire({ title: 'Batalkan Cucian?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#e11d48', confirmButtonText: 'Ya, Bersihkan' }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    photoData.value = null;
                    buktiTransferData.value = null;
                    resetEstimasiSelesai(); // 🚀 Reset juga saat tombol clear ditekan
                }
            }),
        totalTagihan,
        kembalian,
