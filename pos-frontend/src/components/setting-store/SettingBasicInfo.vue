		...
		<div class="border-t border-slate-100 pt-6 space-y-4 w-full">
			...
			<div class="col-span-1 sm:col-span-2 md:col-span-12 mt-4">
				<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
					<label class="text-[10px] font-black uppercase tracking-widest ml-1 flex items-center gap-1.5 text-slate-700">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
							<path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
						</svg>
						Radar Geofencing: Geser Pin Jika Posisi Ruko Kurang Akurat
					</label>

					<button type="button" @click.prevent="dapatkanLokasiGpsUser" :disabled="isTrackingGps" class="sm:self-end px-4 py-2 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-md active:scale-95 shrink-0">
						<svg xmlns="http://www.w3.org/2000/svg" :class="{ 'animate-spin': isTrackingGps }" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
				...
				<div ref="mapContainer" class="w-full h-60 bg-slate-50 rounded-2xl border border-slate-200 shadow-inner z-10 overflow-hidden transition-all ring-4 ring-slate-950/5"></div>

				<div class="mt-2 text-[9px] font-bold text-slate-400 text-right tracking-wider">
					Radar Koordinat:
					<span class="text-slate-600 font-mono font-black normal-case ml-1">
