...
// 🚀 DYNAMIC ENDPOINT ENGINE: Deteksi kasta industri toko secara realtime bray!
const currentIndustry = (localStorage.getItem('user_industry') || 'retail').toLowerCase();
const endpointPrefix = computed(() => {
    return (currentIndustry === 'jasa' || currentIndustry === 'laundry') ? '/laundry' : '/retail';
});

onMounted(async () => {
    try {
        // Tembak endpoint dinamis menyesuaikan tipe ruko merchant bray
        const response = await api.get(`${endpointPrefix.value}/store/settings`);
...
const saveSettings = async () => {
    ...
    try {
        ...
        // Simpan via url dinamis kasta tertinggi bray
        await api.put(`${endpointPrefix.value}/store/settings`, formData);
...
const testConnection = async () => {
    ...
    try {
        // Kirim test message nembak sirkuit Go yang sesuai bray bray!
        const response = await api.post(`${endpointPrefix.value}/whatsapp/test`, {
        ...
        if (response.data.status) {
            ...
            Swal.fire('Gagal', response.data.reason || 'Cek kembali validitas token!', 'error');
        }
    } catch (error) {
