...
// 🚀 SINKRONISASI CORES MAP: Ubah value-nya jadi UPPERCASE murni biar sinkron ama Golang bray!
const detailOptions = {
    Retail: [
        ...
        { value: 'RETAIL_LAINNYA', label: 'Retail Retail Bisnis Lainnya', isReady: true }
    ],
    'F&B': [
    ...
    Jasa: [
        { value: 'LAUNDRY', label: 'Jasa Laundry', isReady: true }, // 🧺 MODUL UTAMA KASTA TERTINGGI LU BRAY!
        { value: 'BARBER', label: 'Barbershop / Salon Kecantikan (Soon)', isReady: false }, // ✂️ Alur: Booking jam + Komisi Stylist
        { value: 'BENGKEL', label: 'Bengkel Otomotif / Service (Soon)', isReady: false }, // 🔧 Alur: Pelat Nomor + Sparepart + Montir
        { value: 'CAR_WASH', label: 'Cuci Mobil & Motor (Soon)', isReady: false },        // 🚗 Alur: Jenis Kendaraan + Antrean Hidrolik
        { value: 'CLINIC', label: 'Klinik / Praktek Dokter Mandiri (Soon)', isReady: false },// 🩺 Alur: Rekam Medis + Reservasi Pasien
        { value: 'SPA_REFLEXOLOGY', label: 'Spa & Tempat Refleksi (Soon)', isReady: false },// 💆‍♂️ Alur: Durasi Jam + Kamar + Terapis
...
// Watcher pintar: Auto-select opsi pertama yang "IsReady" biar user kaga bisa milih opsi zonk bray
watch(
    () => props.formData.kategori_bisnis,
    (newVal) => {
        const options = detailOptions[newVal] || [];
        const readyOption = options.find(o => o.isReady) || options[0];
        props.formData.detail_bisnis = readyOption ? readyOption.value : '';
    },
    { immediate: true }
);
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
            <div class="md:col-span-2">
                ...
                <input v-model="formData.nama_toko" type="text" required class="input-modern text-lg" placeholder="Contoh: Laundry Barokah..." />
            </div>
            ...
            <div class="md:col-span-2 animate-[fadeIn_0.3s_ease-out]">
                <label class="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1 mb-2 flex items-center gap-2">
                    ...
                    Spesifikasi Model Bisnis {{ formData.kategori_bisnis }}
                </label>
                <div class="relative">
                    <select v-model="formData.detail_bisnis" required class="input-modern bg-white cursor-pointer appearance-none text-indigo-900 border-slate-200 focus:border-indigo-500">
                        <option 
                            v-for="opt in detailOptions[formData.kategori_bisnis]" 
                            :key="opt.value" 
                            :value="opt.value" 
