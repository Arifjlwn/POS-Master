...
watch(
  ...
  async (newId) => {
    ...
    listKelurahan.value = [];
    form.value.provinsi =
      listProvinsi.value.find((p) => p.id === newId)?.name || "";
    if (newId) {
      try {
        const res = await fetch(
          `https://ibnux.github.io/data-indonesia/kabupaten/${newId}.json`,
        );
...
watch(
  ...
  async (newId) => {
    ...
    if (newId) {
      try {
        const res = await fetch(
          `https://ibnux.github.io/data-indonesia/kecamatan/${newId}.json`,
        );
        const data = await res.json();
...
watch(
  ...
  async (newId) => {
    ...
    form.value.kecamatan =
      listKecamatan.value.find((p) => p.id === newId)?.name || "";
    if (newId) {
      try {
        const res = await fetch(
          `https://ibnux.github.io/data-indonesia/kelurahan/${newId}.json`,
        );
        const data = await res.json();
...
watch(
  ...
  (newId) => {
    form.value.kelurahan =
      listKelurahan.value.find((p) => p.id === newId)?.name || "";
  },
);
...
const activeKategori = computed(() => {
  return (
    kategoriOptions.find((k) => k.id === form.value.kategori_bisnis) ||
    kategoriOptions[0]
  );
});
...
const submit = async () => {
  if (!regIds.value.kelurahan) {
    return Swal.fire(
      "Data Kurang!",
      "Harap lengkapi pilihan Kelurahan / Desa!",
      "warning",
    );
  }
  ...
  try {
    const finalTipeBisnis = String(
      form.value.detail_bisnis || form.value.kategori_bisnis,
    ).toLowerCase();

    // 🚀 AMBIL ULANG DARI LOCAL STORAGE TEPAT SEBELUM SUBMIT BIAR FRESH!
    const currentPendingIndustry =
      localStorage.getItem("pendingIndustry") || "retail";
    const currentPendingPlan = (
      localStorage.getItem("pendingPlan") || "trial"
    ).toLowerCase();
    ...
    if (
      currentPendingPlan === "basic" ||
      currentPendingPlan === "pro" ||
      currentPendingPlan === "premium"
    ) {
      ...
      try {
        ...
        window.snap.pay(snapToken, {
          ...
          onPending: function (result) {
            Swal.fire(
              "Menunggu Pembayaran",
              "Segera selesaikan transaksi Anda sebelum invoice kedaluwarsa.",
              "info",
            );
          ...
          onClose: function () {
            Swal.fire(
              "Pembayaran Ditunda",
              "Silakan selesaikan invoice Anda nanti di halaman akun.",
              "warning",
            );
            router.push("/retail/account");
          },
...
<template>
  <div
    class="min-h-screen bg-[#F8FAFC] flex flex-col justify-center py-10 md:py-16 sm:px-6 lg:px-8 font-sans relative overflow-hidden"
  >
    <div
      class="absolute -top-24 -left-24 w-[30rem] h-[30rem] bg-indigo-200/40 rounded-full blur-3xl pointer-events-none"
    ></div>
    <div
      class="absolute -bottom-24 -right-24 w-[30rem] h-[30rem] bg-blue-200/40 rounded-full blur-3xl pointer-events-none"
    ></div>

    <div
      class="sm:mx-auto sm:w-full sm:max-w-2xl text-center relative z-10 px-4"
    >
      <div
        class="w-20 h-20 bg-gradient-to-br from-indigo-600 to-blue-600 rounded-[24px] flex items-center justify-center mx-auto shadow-2xl shadow-indigo-200 mb-6 transform -rotate-6 transition-transform hover:rotate-0 duration-500 border-4 border-white"
      >
        <svg
      ...
      <h2
        class="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter"
      >
        Setup Infrastruktur
        <span class="text-indigo-600">Bisnis</span>
      </h2>
      <p
        class="mt-3 text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-[0.2em]"
      >
        Konfigurasi Sistem Menyesuaikan Alur Kerja Anda
      </p>

      <div
        class="mt-4 inline-flex items-center gap-2 bg-indigo-100 text-indigo-700 px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest shadow-sm"
      >
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-3xl px-4 relative z-10">
      <div
        class="bg-white/90 backdrop-blur-xl p-6 md:p-10 shadow-2xl shadow-slate-200/50 rounded-[32px] md:rounded-[40px] border border-white"
      >
        <form @submit.prevent="submit" class="flex flex-col gap-10">
          <div class="flex flex-col gap-6">
            <div class="flex items-center gap-3 border-b border-slate-100 pb-3">
              <div
                class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xs border border-indigo-100 shadow-sm"
              >
                1
              </div>
              <h3
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
              <div class="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="Contoh: Indomaret, Laundry Bersih, dsb..."
                />
              </div>
              ...
              <div class="md:col-span-2">
                ...
                <div
                  class="p-4 rounded-[20px] border-2 border-indigo-200 bg-indigo-50/50 flex items-center gap-4 select-none opacity-90"
                >
                  <div
                    class="w-12 h-12 bg-white rounded-xl shadow-sm border border-indigo-100 flex items-center justify-center"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div class="flex flex-col">
                    <span
                      class="font-black text-slate-800 uppercase tracking-widest"
                      >{{ activeKategori.label }}</span
                    >
                    <span
                      class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest"
                      >Pilihan Dari Landing Page</span
                    >
                  </div>
                  <div
                    class="ml-auto text-slate-400"
                    title="Kategori telah dikunci"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-5 h-5"
              ...
              <div class="md:col-span-2 animate-[fadeIn_0.3s_ease-out]">
                <label
                  class="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1 mb-2 flex items-center gap-2"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="w-3.5 h-3.5"
                ...
                <div class="relative">
                  <select
                    v-model="form.detail_bisnis"
                    required
                    class="input-modern bg-slate-50/50 cursor-pointer appearance-none text-indigo-900 border-indigo-100"
                  >
                    <option
              ...
              <div class="md:col-span-2">
                ...
                <div
                  class="flex items-center bg-white border-2 border-slate-200 rounded-2xl focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all shadow-sm overflow-hidden"
                >
                  <div
                    class="pl-5 pr-4 py-4 bg-slate-50 border-r border-slate-200 flex items-center justify-center select-none"
                  >
          ...
          <div class="flex flex-col gap-6 pt-4 border-t border-slate-100">
            <div class="flex items-center gap-3 border-b border-slate-100 pb-3">
              <div
                class="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600 font-black text-xs border border-emerald-100 shadow-sm"
              >
                2
              </div>
              <h3
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
              <div class="md:col-span-2">
                ...
                <textarea
                  ...
                  rows="2"
                  required
                  class="input-modern resize-none uppercase"
                  placeholder="Contoh: Jl. Jendral Sudirman Kav 21, RT 01 / RW 02..."
                ></textarea>
              </div>
              ...
              <div class="relative">
                ...
                <select
                  ...
                  required
                  class="input-modern cursor-pointer appearance-none"
                >
                  <option value="" disabled selected hidden>
                    Pilih Provinsi...
                  </option>
                  <option
                    v-for="prov in listProvinsi"
                    :key="prov.id"
                    :value="prov.id"
                  >
                    {{ prov.name }}
                ...
                <div
                  class="absolute inset-y-0 right-5 mt-7 flex items-center pointer-events-none"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <div class="relative">
                ...
                <select
                  v-model="regIds.kota"
                  :disabled="!regIds.provinsi"
                  required
                  class="input-modern cursor-pointer appearance-none disabled:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <option value="" disabled selected hidden>
                    {{
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="relative">
                  <label
                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block flex items-center gap-1"
                  >
                    <span class="truncate">Desa / Kel</span>
                  </label>
                  <select
                    v-model="regIds.kelurahan"
                    :disabled="!regIds.kecamatan"
                    required
                    class="w-full px-4 py-4 bg-white border-2 border-slate-200 rounded-2xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none font-black text-slate-800 transition-all text-sm cursor-pointer appearance-none disabled:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <option value="" disabled selected hidden>
                      {{
                        regIds.kecamatan
                  ...
                  <div
                    class="absolute inset-y-0 right-4 mt-7 flex items-center pointer-events-none"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-4 h-4 text-slate-400"
                ...
                <div>
                  ...
                  <input
                    ...
                    :placeholder="
                      regIds.kelurahan ? 'Ketik Kode Pos...' : 'Ketik Kode Pos'
                    "
                  />
                </div>
              </div>
          ...
          <div class="pt-6 mt-2 border-t border-slate-100">
            <button type="submit" :disabled="isLoading" class="btn-submit">
              ...
              <template v-else>
                <div
                  class="w-5 h-5 border-4 border-white/30 border-t-white rounded-full animate-spin mr-3"
                ></div>
      ...
      <p
        class="mt-10 mb-6 text-center text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]"
      >
        NEXA POS Operations &copy; 2026
      </p>
    </div>
  </div>
</template>
