...
import api from '../../api.js'; // Pastikan path axios api.js lu bener bray
...
const handleAdminLogin = async () => {
    ...
    try {
        ...
        // Simpan token kedaulatan root admin bray
        ...
        Swal.fire({
            ...
            router.push('/admin/dashboard'); // Alihkan ke halaman dashboard admin nanti bray
        });
    } catch (err) {
        Swal.fire({
            icon: 'error',
    ...
    <div class="min-h-screen bg-[#0B0F19] flex items-center justify-center p-4 font-sans select-none relative overflow-hidden">
        ...
        <div class="w-full max-w-md bg-[#131B2E] border border-slate-800 rounded-[32px] p-8 md:p-10 shadow-2xl relative z-10 transition-all duration-300">
            <div class="text-center mb-8">
                <span class="px-3 py-1 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-[10px] font-black uppercase tracking-widest">
                    Pusat Otoritas Tertinggi
                </span>
                <h1 class="text-2xl font-black text-white tracking-tight mt-3">ARZURA POS FOUNDER</h1>
                <p class="text-slate-400 text-xs font-medium mt-1">Silakan verifikasi kunci enkripsi admin Anda bray.</p>
            </div>
            ...
            <form @submit.prevent="handleAdminLogin" class="space-y-5">
                <div>
                    ...
                    <input 
                        ...
                        type="email" 
                        required 
                        placeholder="founder@arzura.com" 
                        class="w-full px-5 py-4 bg-[#0B0F19] border border-slate-800 rounded-2xl text-white text-sm font-bold focus:outline-none focus:border-indigo-500 transition-all placeholder:text-slate-600"
                    />
                </div>
                ...
                <button 
                    type="submit" 
                    :disabled="isLoading" 
                    class="w-full py-4 mt-2 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] disabled:opacity-50 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-2 relative overflow-hidden"
                >
