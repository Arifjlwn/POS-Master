		...
		<div class="w-full max-w-md relative group z-10">
			...
			<div class="bg-white rounded-[40px] p-8 md:p-12 shadow-2xl relative border border-white flex flex-col">
				<div class="text-center mb-8">
					...
					<p class="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">Buat password baru akun manajemen anda</p>
				</div>
				...
				<form @submit.prevent="handleResetPassword" class="space-y-5">
					<div class="space-y-2">
						...
						<div class="relative group">
							<div class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors z-10">
								<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
									<rect width="18" height="11" x="3" y="11" rx="2" ry="2" />
									<path d="M7 11V7a5 5 0 0 1 10 0v4" />
								</svg>
							</div>
							<input v-model="password" :type="showPassword ? 'text' : 'password'" required class="w-full pl-12 pr-12 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:border-indigo-600 font-bold text-slate-800 outline-none transition-all placeholder:text-slate-300" placeholder="••••••••" />
							<button type="button" @click="showPassword = !showPassword" class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-indigo-600 transition-all z-10">
						...
						<div class="flex gap-2 mt-2 px-1">
							<div class="flex items-center gap-1 text-[9px] font-bold" :class="password.length >= 6 ? 'text-emerald-500' : 'text-slate-300'">
								<svg v-if="password.length >= 6" class="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="4">
									<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
								</svg>
								6 Karakter
							</div>
							<div class="flex items-center gap-1 text-[9px] font-bold" :class="hasUppercase ? 'text-emerald-500' : 'text-slate-300'">
								<svg v-if="hasUppercase" class="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="4">
									<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
								</svg>
								Huruf Besar
							</div>
							<div class="flex items-center gap-1 text-[9px] font-bold" :class="hasNumber ? 'text-emerald-500' : 'text-slate-300'">
								<svg v-if="hasNumber" class="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="4">
									<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
								</svg>
								Angka
							</div>
					...
					<div class="space-y-2">
						...
						<div class="relative group">
							...
							<input v-model="confirmPassword" :type="showPassword ? 'text' : 'password'" required class="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:border-indigo-600 font-bold text-slate-800 outline-none transition-all placeholder:text-slate-300" placeholder="••••••••" :class="isMatch ? 'border-emerald-100 bg-emerald-50/20 text-emerald-900' : ''" />
						</div>
						<p v-if="isMatch" class="text-[9px] text-emerald-600 font-bold mt-1 ml-1 uppercase tracking-widest flex items-center gap-1">
					...
					<button type="submit" :disabled="isLoading || !isPasswordValid || !isMatch" class="w-full py-5 rounded-2xl bg-indigo-600 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-200 hover:bg-slate-900 transition-all active:scale-95 disabled:opacity-40 disabled:pointer-events-none flex items-center justify-center gap-3 mt-6">
				...
				<button @click="router.push('/login')" class="mt-6 text-[10px] font-black text-slate-300 uppercase tracking-widest hover:text-slate-500 transition-colors focus:outline-none">Batal & Login</button>
			</div>
		</div>
	</div>
</template>
