...
// 🚀 COMPUTED FILTER SAKTI: Singkirkan paket Trial jika Owner sudah punya toko riil di DB
  ...
  <div class="min-h-screen bg-[#F8FAFC] flex flex-col items-center py-12 px-4 md:px-8 font-sans relative overflow-x-hidden select-none">
    ...
    <div class="w-full max-w-6xl z-10 flex flex-col items-center animate-fade-in-up">
      ...
      <div v-if="stores.length > 0" class="w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full relative z-10 items-stretch">
          <div
            ...
            class="group bg-white rounded-[32px] p-6 border border-slate-100 shadow-2xs hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cubic-bezier-card cursor-pointer flex flex-col relative overflow-hidden justify-between"
          >
            <div
            ...
            <div>
              <div class="flex items-start justify-between mb-6">
                ...
                <span
                  class="text-[10px] font-black uppercase tracking-widest bg-slate-50 border border-slate-200/60 px-3 py-1.5 rounded-xl transition-all duration-300 group-hover:bg-indigo-50 group-hover:text-indigo-600 group-hover:border-indigo-100"
                >
                  {{ getStoreLabel(idx) }}
                </span>
              </div>
              ...
              <div class="mb-6">
                <h3
                  class="text-xl font-black text-slate-800 mb-1.5 line-clamp-1 uppercase tracking-tight group-hover:text-indigo-600 transition-colors duration-300"
                >
                  {{ store.nama_toko }}
                </h3>
            ...
            <div class="mt-auto flex items-center justify-between border-t border-slate-50/80 pt-4">
              ...
              <div
                class="text-xs font-black text-slate-700 group-hover:text-indigo-600 uppercase tracking-widest flex items-center gap-0.5 transition-colors duration-300"
              >
                Buka Sesi Kasir
                <svg
                  class="w-4 h-4 group-hover:translate-x-1.5 transition-transform duration-300 ease-out"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="3"
          ...
          <div
            ...
            class="bg-white rounded-[32px] p-6 border-2 border-dashed border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/10 hover:-translate-y-1.5 transition-all duration-500 cubic-bezier-card cursor-pointer flex flex-col items-center justify-center text-center min-h-[210px] group shadow-3xs"
          >
            <div
      ...
      <div v-else class="w-full max-w-xl text-center py-12 px-6 bg-white rounded-[40px] border border-slate-100 shadow-xl">
        ...
        <button
          @click="showPricingModal = true"
          class="w-full py-4 bg-indigo-600 hover:bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all duration-300 active:scale-95"
        >
    ...
    <Transition name="modal-fade">
      <div v-if="showPricingModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 overflow-y-auto">
        ...
        <div
          class="bg-white w-full max-w-6xl rounded-[40px] shadow-2xl p-6 md:p-10 relative border border-slate-100 my-auto max-h-[92vh] overflow-y-auto custom-scrollbar flex flex-col items-center z-10 transform transition-all duration-500 cubic-bezier-modal"
        >
          <button
          ...
          <div
            class="bg-white/80 backdrop-blur-md p-1.5 rounded-[24px] border border-slate-200/80 flex w-full max-w-xl shadow-xl shadow-slate-100/50 mb-12 shrink-0"
          >
            <button
          ...
          <div class="flex flex-wrap justify-center gap-6 w-full items-stretch animate-fade-in-cards">
            ...
            >
              <div
                v-if="plan.id === 'pro'"
                class="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl whitespace-nowrap"
              >
                REKOMENDASI UMKM
              </div>
              <div
...
/* 🚀 FIX TRANSISI MODAL ANTI-BLINK & SMOOTH DISMISSAL */
