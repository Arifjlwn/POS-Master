...
// ==============================================================
// CONFIG NAMA BRAND APLIKASI (Ubah di sini untuk ganti nama)
// ==============================================================
...
onMounted(() => {
    const queryInd = route.query.industry || localStorage.getItem('pendingIndustry');
    const queryPlan = route.query.plan || localStorage.getItem('pendingPlan');

    if (queryInd) {
        selectedIndustry.value = queryInd;
        localStorage.setItem('pendingIndustry', queryInd);
    }
    if (queryPlan) {
        selectedPlan.value = queryPlan;
        localStorage.setItem('pendingPlan', queryPlan);
    }
});
...
const handleRegister = async () => {
    ...
    try {
        ...
        // KATA-KATANYA DIUBAH BIAR GA MEMBINGUNGKAN (Karena OTP belum dikirim)
        Swal.fire({ 
            icon: 'success', 
            title: 'Pendaftaran Berhasil!', 
            text: 'Akun Owner Anda telah berhasil dibuat. Silakan pilih metode aktivasi akun Anda.', 
            confirmButtonColor: '#4f46e5' 
        });

        router.push({ 
            path: '/select-verify', 
            query: { email: form.value.email, phone: `62${form.value.no_hp}` } 
        });
    } catch (error) {
        ...
        <div class="w-full max-w-2xl relative">
            <div class="bg-white rounded-[40px] p-8 md:p-12 shadow-2xl relative border border-white">
                ...
                <div class="text-center mb-10">
                    <div class="font-black text-3xl tracking-tighter uppercase cursor-pointer flex items-center justify-center gap-1.5" @click="goHome">
                        {{ BRAND_NAME }}<span class="text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg text-xs tracking-normal normal-case font-bold border border-indigo-100">POS</span>
                    </div>
                    <p class="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em] mt-3">Daftarkan akun owner untuk mulai bisnis</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative group">
                                ...
                                <input v-model="form.confirmPassword" type="password" required class="input-field" :class="isMatch ? 'border-emerald-100 bg-emerald-50/30 text-emerald-900' : ''" placeholder="Ulangi Password">
                            </div>
                ...
                <div class="mt-8 text-center flex flex-col sm:flex-row items-center justify-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-tight border-t border-slate-100 pt-6">
                    <div>
                        Sudah punya akun? <router-link to="/login" class="text-indigo-600 hover:underline">Masuk Sekarang</router-link>
                    </div>
                    <span class="hidden sm:inline opacity-30">•</span>
                    <div>
                        <button @click="goHome" class="text-slate-500 hover:text-indigo-600 hover:underline flex items-center gap-1 focus:outline-none">
...
.icon-container { @apply absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors z-10; }
