...
const handleLogin = async () => {
	...
	try {
		...
		// Keluarkan gembok modal alert secara presisi
		Swal.fire({
			icon: 'error',
			title: 'Akses Ditolak',
			text: msg,
...
<template>
	<div class="min-h-screen flex items-center justify-center bg-[#F8FAFC] font-sans p-4 relative overflow-hidden">
		<div class="absolute -top-24 -left-24 w multiplicative-96 h-96 bg-blue-100/50 rounded-full blur-3xl"></div>
		<div class="absolute -bottom-24 -right-24 w-96 h-96 bg-indigo-100/50 rounded-full blur-3xl"></div>

		<div :class="{ 'animate-shake': isErrorShake }" class="w-full max-w-md relative group mt-10 md:mt-0 z-10 transition-transform">
			...
			<div class="bg-white rounded-[40px] p-8 md:p-12 shadow-2xl relative border border-white flex flex-col">
				...
				<form @submit.prevent="handleLogin" class="space-y-5">
					...
					<button type="submit" :disabled="isLoading" class="w-full py-4 rounded-2xl bg-blue-600 text-white font-black text-sm uppercase tracking-widest shadow-xl shadow-blue-200 hover:bg-slate-900 hover:shadow-slate-200 transition-all active:scale-95 disabled:opacity-75 flex items-center justify-center gap-3 mt-4 relative overflow-hidden">
						<div :class="{ 'opacity-0 invisible': isLoading }" class="flex items-center gap-3 transition-all duration-200">
							MASUK
						...
						<div v-if="isLoading" class="absolute inset-0 flex items-center justify-center gap-3 bg-blue-600 group-hover:bg-slate-900 transition-colors">
							<svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
								<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
								<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
							</svg>
							<span class="text-white font-black text-sm uppercase tracking-widest">SINKRONISASI...</span>
						</div>
					</button>
				</form>
