				...
				<form @submit.prevent="handleCheckAccount" class="space-y-6 text-left">
					<div class="space-y-2">
						<div class="relative group">
							...
							<input v-model="identifier" type="text" required class="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:bg-white focus:border-indigo-600 font-bold text-slate-800 outline-none transition-all placeholder:text-slate-300" placeholder="Email atau No. WA Anda"> 
						</div>
					</div>

					<button type="submit" :disabled="isLoading || !identifier" class="w-full py-5 rounded-2xl bg-indigo-600 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-200 hover:bg-slate-900 transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none">
						<span v-if="isLoading">MENCARI AKUN...</span>
						<span v-else>LANJUTKAN VIA SECURE OTP</span>
					</button>
				</form>

				<button @click="goBack" class="mt-6 w-full py-4 rounded-xl bg-slate-50 border border-slate-100 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-red-500 hover:bg-red-50 hover:border-red-100 transition-all flex items-center justify-center gap-2 focus:outline-none">
					BATAL & KEMBALI
				</button>
