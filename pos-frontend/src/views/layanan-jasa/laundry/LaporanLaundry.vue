...
<template>
    <SidebarLaundry>
        <div class="flex-1 flex flex-col h-full bg-slate-50/50 overflow-hidden relative">
            <div class="p-5 lg:p-8 shrink-0 bg-white border-b border-slate-200 flex flex-col gap-5 md:gap-6 z-10 shadow-sm relative">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-xl md:text-2xl font-black tracking-tighter uppercase text-slate-800 leading-tight">Analitik Enterprise</h1>
                            <p class="text-[10px] font-black text-slate-400 mt-0.5 uppercase tracking-[0.2em]">Pusat Kontrol Owner</p>
                        </div>
                    </div>
                    
                    <button class="w-full md:w-auto bg-slate-900 hover:bg-indigo-600 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 shadow-xl shadow-slate-200 transition-all">
                ...
                <div class="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between border-t border-slate-100 pt-5">
                    <div class="flex gap-2 w-full xl:w-auto overflow-x-auto hide-scrollbar pb-2 xl:pb-0">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-5 lg:p-8 pb-24">
                <div v-if="isLoading && riwayat.length === 0" class="flex flex-col items-center justify-center py-32 text-slate-400">
                ...
                <template v-else>
                    <div v-if="activeTab === 'ringkasan'" class="space-y-6 lg:space-y-8 animate-[fadeIn_0.3s_ease-out]">
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 lg:gap-6">
                            ...
                            <div class="bg-white p-6 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-200 shadow-sm">
                            ...
                            <div class="bg-white p-6 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-200 shadow-sm sm:col-span-2 xl:col-span-1">
                    ...
                    <div v-if="activeTab === 'keuangan'" class="animate-[fadeIn_0.3s_ease-out] space-y-6 lg:space-y-8">
                        ...
                        <div class="bg-white border border-slate-200 rounded-[32px] shadow-sm overflow-hidden flex flex-col">
                            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                <input v-model="searchQuery" type="text" placeholder="Cari Resi..." class="px-4 py-2 bg-white border-2 border-slate-200 rounded-xl font-bold text-xs outline-none">
                            </div>
                            <div class="overflow-x-auto">
                                <table class="w-full text-left whitespace-nowrap">
                                    <thead>
                                        <tr class="bg-white text-[9px] uppercase tracking-[0.2em] text-slate-400 border-b-2 border-slate-100">
                                            <th class="p-5 font-black">Data Invoice</th>
                                            <th class="p-5 font-black">Pelanggan</th>
                                            <th class="p-5 font-black">Status Pembayaran</th>
                                            <th class="p-5 font-black">Nominal</th>
                                            <th class="p-5 font-black text-center">Audit</th>
                                        </tr>
                                    </thead>
                                    <tbody class="text-slate-700">
                                        <tr v-for="trx in filteredRiwayat" :key="trx.id" class="border-b border-slate-50 hover:bg-slate-50">
                                            <td class="p-5">
                                            ...
                                            <td class="p-5 font-bold text-xs text-slate-700 uppercase">{{ trx.pelanggan }}</td>
                                            <td class="p-5">
                                            ...
                                            <td class="p-5 font-black text-xs tracking-tight">{{ formatRupiah(trx.total_harga) }}</td>
                    ...
                    <div v-if="activeTab === 'analitik'" class="animate-[fadeIn_0.3s_ease-out]">
                        <div class="bg-white border border-slate-200 rounded-[32px] p-6 lg:p-8 shadow-sm">
                            <h2 class="text-sm font-black text-slate-800 uppercase tracking-widest mb-6">Top 5 Pelanggan VIP</h2>
                            <div class="space-y-4">
                                <div v-for="(vip, index) in vipCustomers" :key="index" class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                    <div class="flex items-center gap-4">
                                        <div class="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center font-black text-xs">{{ index + 1 }}</div>
                                        <div><p class="font-black text-xs uppercase">{{ vip.nama }}</p><p class="text-[10px] font-bold text-slate-400">{{ vip.kunjungan }}x Kunjungan</p></div>
                                    </div>
