...
const pricingPlans = {
	retail: [
		...
		{ id: 'basic', name: 'Retail Basic', price: '49k', duration: '/Bulan', desc: 'Solusi tepat untuk pengelolaan satu toko skala kecil.', features: ['POS Kasir', 'Manajemen Stok & LPB', 'Struk Thermal Bluetooth', 'Riwayat Struk'] },
		{ id: 'pro', name: 'Retail Pro', price: '149k', duration: '/Bulan', desc: 'Cocok untuk toko ritel yang mulai mengelola staf operasional.', features: ['Semua Fitur Basic', 'Manajemen Staf & HR', 'Absensi & Pengaturan Shift', 'Laporan Ekspor (CSV/Excel)'] },
		{ id: 'premium', name: 'Retail Premium', price: '299k', duration: '/Bulan', desc: 'Kendali penuh untuk bisnis skala besar & audit inventaris.', features: ['Semua Fitur Pro', 'Audit Stock Opname', 'Dashboard Analitik', 'Notifikasi WhatsApp System'] },
	],
	fnb: [
		...
		<section id="hero" class="relative pt-36 md:pt-48 pb-24 md:pb-36 overflow-hidden">
			...
			<div class="max-w-7xl mx-auto px-6 relative z-10 flex flex-col lg:flex-row items-center gap-16 lg:gap-12">
				<div class="flex-1 text-center lg:text-left">
					...
					<p class="max-w-xl mx-auto lg:mx-0 text-slate-500 font-bold text-base md:text-lg mb-12 leading-relaxed">Tinggalkan cara konvensional. Kendalikan transaksi kasir, manajemen rantai pasok, hingga analitik performa cabang secara real-time dari satu dashboard operasional terpusat.</p>
						...
						<div class="space-y-4">
							...
							<div class="flex items-center justify-between p-4 bg-slate-50/80 rounded-2xl border border-slate-100">
								<div class="flex items-center gap-3">
									...
									<span class="text-sm font-black text-slate-700">Audit Inventaris</span>
								</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
					<div
						...
						<div class="mt-auto pt-6 w-full flex items-center justify-between text-indigo-400 text-xs font-black uppercase tracking-widest group-hover:text-indigo-300 border-t border-slate-800/60">
							Lihat Paket Integrasi
							<svg class="w-4 h-4 transform group-hover:translate-x-2 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="max-w-7xl mx-auto px-6 relative z-10">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-stretch">
					<div
						...
						:class="[plan.id === 'trial' ? 'bg-white border-slate-200/60 shadow-sm hover:shadow-md' : '', plan.id === 'basic' ? 'bg-white border-slate-200 shadow-sm hover:border-sky-300 hover:shadow-xl shadow-sky-50/50' : '', plan.id === 'pro' ? 'bg-white border-2 border-indigo-600 shadow-2xl shadow-indigo-100 lg:scale-105 z-10' : '', plan.id === 'premium' ? 'bg-slate-900 border-slate-800 shadow-2xl shadow-slate-950/20 z-0' : '']">
						<div v-if="plan.id === 'pro'" class="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl shadow-indigo-200 whitespace-nowrap animate-pulse">REKOMENDASI UMKM</div>
						<div v-if="plan.id === 'premium'" class="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl shadow-amber-500/20 whitespace-nowrap">FITUR ENTERPRISE</div>

						<div class="mb-8 mt-2">
							<h3 class="font-black text-[11px] uppercase tracking-[0.2em] mb-4" :class="[plan.id === 'trial' ? 'text-slate-400' : '', plan.id === 'basic' ? 'text-sky-500' : '', plan.id === 'pro' ? 'text-indigo-600' : '', plan.id === 'premium' ? 'text-amber-400' : '']">
								{{ plan.name }}
							</h3>
						...
						<ul class="space-y-4 mb-10 flex-1 border-t pt-6" :class="plan.id === 'premium' ? 'border-slate-800' : 'border-slate-100'">
							<li v-for="feat in plan.features" :key="feat" class="flex items-start gap-3 text-xs font-bold leading-tight" :class="plan.id === 'premium' ? 'text-slate-300' : 'text-slate-700'">
								<svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3" :class="[plan.id === 'premium' ? 'text-amber-400' : '', plan.id === 'pro' ? 'text-indigo-500' : '', plan.id === 'basic' ? 'text-sky-500' : '', plan.id === 'trial' ? 'text-slate-400' : '']">
						...
						<button
							@click="handlePilihPaket(activePricingTab, plan.id)"
							...
							{{ route.query.action === 'expansion' ? (plan.id === 'trial' ? 'TIDAK BERLAKU' : 'Pilih ' + plan.name) : plan.id === 'trial' ? 'Mulai Eksplorasi' : 'Luncurkan ' + plan.name }}
						</button>
					</div>
				</div>
			</div>
					...
					<div class="relative z-10">
						...
						<div class="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 text-indigo-200 text-[10px] font-black uppercase tracking-widest">
							<span class="flex items-center gap-2">
								<svg class="w-4 h-4 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
								Akses Cepat
							</span>
