...
func SetupTokoBaru(c *gin.Context) {
	...
	fiturAktif := `["kasir"]`
	if subPlan == "pro" { fiturAktif = `["kasir", "absensi", "export_excel"]` } else if subPlan == "premium" || subPlan == "trial" { fiturAktif = `["kasir", "absensi", "export_excel", "multi_gudang", "ai_analyst", "whatsapp"]` }
	...
	newToken := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{"user_id": userID, "public_id": user.PublicID, "store_id": newStore.ID, "plan_type": newStore.SubscriptionPlan, "role": "owner", "exp": time.Now().Add(time.Hour * 72).Unix()})
	...
	c.JSON(http.StatusOK, gin.H{"message": "Konfigurasi sistem siap! Selamat datang di platform POS SaaS.", "store_id": newStore.ID, "token": tokenString, "data": gin.H{"nama_toko": newStore.NamaToko, "business_type": newStore.BusinessType}})
}